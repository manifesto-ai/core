...
> - **v0.3.2: Core alignment — available, fail, stop, call policy (FDR-MEL-058 ~ 061)**
...
13. [Flow Control (v0.3.2)](#13-flow-control-v032)
...
**v0.3.2 Note:** `$acc` is removed. `reduce` is forbidden. See FDR-MEL-062.
...
#### 9.1.9 Primitive Aggregation Functions (v0.3.2)
...
// ❌ array.reduce is FORBIDDEN (v0.3.2)
...
Core's `call` FlowNode exists but is **NOT exposed in MEL v0.3.2**.
...
### 15.1 From MEL v0.3.1 to v0.3.2
...
```mel
// v0.3.2 NEW: available, fail, stop
action withdraw() available when gt(balance, 0) {
  when isNull(account) {
...
**Non-Breaking:** All v0.3.1 code is valid v0.3.2 code. New features are additive.
...
MEL v0.3.2 is designed with the following AI-Native principles:
...
| **Action availability** | `available when <Expr>` for declarative preconditions (v0.3.2) |
| **Errors are values** | `fail` is FlowNode, not Effect — Core decides (v0.3.2) |
| **Early exit vs pending** | `stop` is early-exit, "waiting" forbidden (v0.3.2) |
| **call hidden** | Core retains call, MEL hides it for simplicity (v0.3.2) |
...
29. **available is pure** — no Effects, no $system.* in availability conditions (v0.3.2)
30. **fail must be guarded** — unconditional fail is compile error (v0.3.2)
31. **stop must be guarded** — unconditional stop is compile error (v0.3.2)
32. **stop ≠ waiting** — "Waiting for..." messages are lint errors (v0.3.2)

---

*End of MEL SPEC v0.3.2*
