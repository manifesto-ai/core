...
/**
 * Raw statistics collected from traces.
 */
interface RawStats {
  runs: number;
  successCount: number;
  ...
  byLevel: Map<
    NecessityLevel,
    { runs: number; successCount: number; totalSteps: number; totalDurationMs: number }
  >;
  ...
  llm: {
    ...
    approved: number;
    rejected: number;
    byRole: Map<LLMRole, { proposals: number; approved: number; rejected: number }>;
  };
  totalProposals: number;
}
 ...
 */
function collectStats(traces: LabTrace[]): RawStats {
 ...
 */
function processTrace(trace: LabTrace, stats: RawStats): void {
  ...
  levelStats.runs++;
  if (isSuccess) levelStats.successCount++;
 ...
 */
function processEvent(event: LabTraceEvent, stats: RawStats): void {
  switch (event.type) {
    case "proposal":
      processProposalEvent(event as ProposalTraceEvent, stats);
      break;
    case "authority.decision":
      processDecisionEvent(event as AuthorityDecisionTraceEvent, stats);
      break;
    case "hitl":
      processHITLEvent(event as HITLTraceEvent, stats);
      break;
    case "failure.explanation":
      processFailureEvent(event as FailureExplanationTraceEvent, stats);
      break;
  }
}
 ...
 */
function processProposalEvent(event: ProposalTraceEvent, stats: RawStats): void {
 ...
 */
function processDecisionEvent(
  event: AuthorityDecisionTraceEvent,
  stats: RawStats
): void {
 ...
 * Process HITL event.
 */
function processHITLEvent(event: HITLTraceEvent, stats: RawStats): void {
  switch (event.action) {
    case "pending":
...
function processFailureEvent(
  event: FailureExplanationTraceEvent,
  stats: RawStats
): void {
 ...
 */
function countSteps(events: LabTraceEvent[]): number {
 ...
 */
function detectLLMRole(actorId: string): LLMRole {
 ...
 */
function buildSummary(stats: RawStats): TraceSummary {
  return {
    runs: stats.runs,
    ...
    failureReasons: Object.fromEntries(stats.failureReasons) as Record<
      FailureReason,
      number
    >,
...
function buildLevelSummaries(
  byLevel: Map<
    ...
    { runs: number; successCount: number; totalSteps: number; totalDurationMs: number }
  >
): Record<NecessityLevel, LevelSummary> {
  const result: Record<number, LevelSummary> = {};
  ...
  for (const [level, levelStats] of byLevel) {
    result[level] = {
      ...
      avgDurationMs:
        levelStats.runs > 0 ? levelStats.totalDurationMs / levelStats.runs : 0,
    };
  }

  return result as Record<NecessityLevel, LevelSummary>;
}

/**
 * Build HITL summary.
 */
function buildHITLSummary(stats: RawStats): HITLSummary {
  ...
  return {
    ...
    avgDecisionTimeMs:
      hitl.decisionCount > 0 ? hitl.totalDecisionTimeMs / hitl.decisionCount : 0,
 ...
 * Build LLM summary.
 */
function buildLLMSummary(stats: RawStats): LLMSummary {
  const llm = stats.llm;

  const byRole: LLMSummary["byRole"] = {
 ...
 */
function createEmptySummary(): TraceSummary {
  return {
    runs: 0,
    successRate: 0,
    avgSteps: 0,
    ...
    failureReasons: {} as Record<FailureReason, number>,
    hitl: {
      triggered: 0,
      approved: 0,
      rejected: 0,
...
export function formatSummary(summary: TraceSummary): string {
  ...
  for (let level = 0; level <= 3; level++) {
    const levelSummary = summary.byLevel[level as NecessityLevel];
    if (levelSummary.runs > 0) {
      lines.push(
        `  Level ${level}: ${levelSummary.runs} runs, ${(levelSummary.successRate * 100).toFixed(1)}% success`
      );
    }
  }
  lines.push("");
