...
import type { Settled, Fulfilled, ErrorValue } from "../types/index.js";
 ...
 * ```
 */
export function collectErrors<T extends Record<string, Settled<unknown>>>(
  results: T,
  path: string
): Patch[] {
  const errors: Record<string, ErrorValue> = {};
 ...
 * ```
 */
export function collectFulfilled<T extends Record<string, Settled<unknown>>>(
  results: T
): {
