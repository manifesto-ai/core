...
| 함수 | 설명 | 예시 |
|------|------|------|
| `length` | 길이 | `['length', ['get', 'data.items']]` |
| `at` | 인덱스 접근 | `['at', ['get', 'data.items'], 0]` |
| `first` | 첫 번째 요소 | `['first', ['get', 'data.items']]` |
| `last` | 마지막 요소 | `['last', ['get', 'data.items']]` |
| `includes` | 포함 여부 | `['includes', ['get', 'data.tags'], 'sale']` |
| `indexOf` | 인덱스 찾기 | `['indexOf', ['get', 'data.items'], 'A']` |
