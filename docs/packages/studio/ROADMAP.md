...
Phase 2: DAG 시각화           ████████████ 100% ✅
Phase 2.5-2.8: Core 통합      ████████████ 100% ✅
Phase 3: 시나리오 테스트      ████████████ 100% ✅
Phase 4: Compiler 연동        ░░░░░░░░░░░░   0%
Phase 5: 익스포트             ████████████ 100% ✅
```

**테스트 현황**:
- Studio: 305 tests passing (12 test files)
- Core: 791 tests passing (25 test files)
- **Total: 1,096 tests**
...
## Phase 2.5-2.8: Core 통합 ✅

Studio가 Core의 Expression Evaluator를 직접 사용하도록 통합.
...
| 항목 | 상태 | 파일/위치 |
|------|------|-----------|
| Core First 원칙 문서화 | ✅ | `CLAUDE.md` |
| Studio evaluator → Core import | ✅ | `expression-evaluator.ts` |
| 다형성 연산자 (7개) | ✅ | Core `evaluator.ts` |
| FP 배열 함수 (12개) | ✅ | Core `evaluator.ts` |
| 객체 조작 함수 (assoc, dissoc, merge) | ✅ | Core `evaluator.ts` |
| UUID 함수 | ✅ | Core `evaluator.ts` |
| 배열 리터럴 지원 | ✅ | Core `evaluator.ts` |
| 문서 업데이트 (EN/KO) | ✅ | `docs/packages/core/` |

**Core 연산자**: 70+ (기존 51개 → 70+개)
...
| 항목 | 상태 | 파일/위치 |
|------|------|-----------|
| 시나리오 타입 정의 | ✅ | `runtime/scenario-types.ts` |
| 시나리오 실행 엔진 | ✅ | `runtime/scenario-executor.ts` |
| Effect 모킹 시스템 | ✅ | `runtime/effect-mocker.ts` |
| 시나리오 패널 UI | ✅ | `components/scenario/ScenarioPanel.tsx` |
| 시나리오 편집기 | ✅ | `components/scenario/ScenarioEditor.tsx` |
| 결과 뷰어 | ✅ | `components/scenario/ScenarioResultView.tsx` |
| Todo App 예시 시나리오 | ✅ | `domain/examples/todo-app.ts` |
| expectFailure 지원 | ✅ | Precondition 실패 테스트 |

**테스트**:
- `scenario-executor.test.ts` (34 tests)
- `effect-mocker.test.ts` (24 tests)
- `scenario-todo-app.test.ts` (11 tests)
...
interface ScenarioStep {
  ...
  expectFailure?: boolean;  // Precondition 실패 예상
}

interface ScenarioAssertion {
  path: string;
  operator: "eq" | "neq" | "gt" | "gte" | "lt" | "lte" | "contains" | "length" | "truthy" | "falsy";
  expected?: unknown;
}
```
...
```
components/scenario/
├── ScenarioPanel.tsx        # 메인 패널 (시나리오 목록)
├── ScenarioEditor.tsx       # Given-When-Then 편집
├── ScenarioResultView.tsx   # 실행 결과 표시
└── index.ts

runtime/
├── scenario-types.ts        # 타입 + Zod 스키마
├── scenario-executor.ts     # 순수 함수: 시나리오 실행
├── scenario-executor.test.ts
├── scenario-todo-app.test.ts  # Todo App 통합 테스트
├── effect-mocker.ts         # Effect 모킹 유틸
└── effect-mocker.test.ts
```

### 3.5 Todo App 시나리오 (7개)

| 시나리오 | 상태 | 설명 |
|----------|------|------|
| Add New Todo | ✅ | 새 할 일 추가 |
| Cannot Add Empty Todo | ✅ | 빈 텍스트 추가 불가 (expectFailure) |
| Toggle Todo Completion | ✅ | 완료 상태 토글 |
| Delete Todo | ✅ | 할 일 삭제 |
| Clear Completed Todos | ✅ | 완료된 항목 제거 |
| Set Filter to Active | ✅ | 필터 변경 |
| Toggle All Todos | ✅ | 전체 토글 |
...
## 다음 단계: Phase 4 (Compiler 연동)

### 개요
- AI 기반 도메인 자동 생성/수정 시스템
- 자연어 → Manifesto 스키마 변환
- 결정론적 검증 루프 (LLM 생성 → validateDomain → 재시도)
