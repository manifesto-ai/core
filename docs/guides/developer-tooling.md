...
- Studio snapshot inspection expects full internal snapshots from `runtime.inspect.canonicalSnapshot()`, not the app-facing result of `snapshot()`.
- Runtime-aware tools should use action `preview()` for current-snapshot dry-runs and `@manifesto-ai/sdk/extensions` for arbitrary full internal snapshots or lower-level typed-intent compatibility seams.
