...
```typescript
world.registerAuthority("schema-authority", {
  evaluate: async (proposal) => {
    if (proposal.intent.type.startsWith("schema:")) {
      // Schema changes always require human approval
      return {
        decision: "pending",
        reason: "Schema changes require human review",
        escalateTo: "admin-authority"
      };
    }
...
- [AI Agent Integration](/guides/ai-agent-integration) — AI integration patterns
