...
| Area | Description |
|------|-------------|
| Host↔World Integration | HostExecutor, WorldStore, Trace mapping |
...
7. Validate effects if `validation.effects='strict'`
...
2. The App **MUST** verify that every required effect type is provided by the App's `effects` registry (including system-reserved handlers).
...
/**
 * Validate schema against App's effects registry.
