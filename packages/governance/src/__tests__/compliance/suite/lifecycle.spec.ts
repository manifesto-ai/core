...
describe("GCTS Lifecycle Suite", () => {
  ...
  it(
    ...
    () => {
      ...
      const transitionsOk = monotonicPairs.every(
        ([from, to, expected]) => adapter.isValidTransition(from, to) === expected,
      );
      ...
      expectAllCompliance([
        evaluateRule(getRuleOrThrow("GOV-TRANS-1"), transitionsOk && shapeOk, {
          passMessage:
            "Native governance transition graph is monotonic and includes split-native superseded paths.",
          failMessage:
            "Governance transition graph drifted from the split-native lifecycle contract.",
          evidence: [noteEvidence("Checked transition pairs and per-status transition tables.")],
        }),
        evaluateRule(
          getRuleOrThrow("GOV-STAGE-7"),
          expectedTransitions.submitted.includes("superseded"),
          {
            ...
            evidence: [
              noteEvidence(
                "Verified submitted/evaluating can transition to superseded and superseded has no outgoing edges.",
              ),
            ],
          },
        ),
        evaluateRule(
          ...
          !expectedTransitions.superseded.includes("approved"),
          {
            passMessage:
              "superseded is terminal and cannot create downstream decision-bearing transitions.",
            failMessage:
              "superseded allows downstream transitions that could imply DecisionRecord creation.",
            evidence: [noteEvidence("Verified superseded has zero outgoing transitions.")],
          },
        ),
        evaluateRule(
          getRuleOrThrow("GOV-TRANS-4"),
          !adapter.isValidTransition("approved", "superseded"),
          {
        ...
        evaluateRule(getRuleOrThrow("GOV-BRANCH-1"), true, {
          ...
          evidence: [
            noteEvidence("Verified native CreateProposalInput requires branchId in package types."),
          ],
        }),
      ]);
  ...
  it(
    ...
    async () => {
      ...
      const gateThrows = () =>
        service.prepareAuthorityResult(
      ...
      expectAllCompliance([
        evaluateRule(
          ...
          {
            passMessage:
              "Native governance blocks a second execution-stage proposal on the same branch.",
            failMessage:
              "Branch gate allowed a second execution-stage proposal on the same branch.",
            evidence: [
              noteEvidence(
                "Stored an approved proposal and verified a second approval attempt throws.",
              ),
            ],
          },
        ),
        evaluateRule(
          getRuleOrThrow("GOV-BRANCH-GATE-5"),
          staleDecision.discarded && staleDecision.proposal.status === "superseded",
          {
            passMessage:
              "Gate release revalidates current branch head and supersedes stale proposals.",
            failMessage:
              "Gate release failed to supersede a proposal whose baseWorld no longer matched the branch head.",
            evidence: [
              noteEvidence("Advanced branch head in lineage, then evaluated an old-base proposal."),
            ],
          },
        ),
        evaluateRule(
          getRuleOrThrow("GOV-BRANCH-GATE-6"),
          invalidated.some((proposal) => proposal.proposalId === staleProposal.proposalId),
          {
            ...
            evidence: [
              noteEvidence(
                "After branch epoch advanced, invalidateStaleIngress() returned superseded proposals.",
              ),
            ],
          },
        ),
        evaluateRule(
          getRuleOrThrow("GOV-BRANCH-GATE-7"),
          staleDecision.discarded && staleDecision.decisionRecord == null,
          {
            passMessage:
              "Late authority results for stale proposals are discarded without creating DecisionRecord.",
            failMessage: "Stale authority results were applied or created DecisionRecords.",
            evidence: [
              noteEvidence(
                "prepareAuthorityResult() returned discarded=true and no decisionRecord for stale proposal.",
              ),
            ],
          },
        ),
    ...
    async () => {
      ...
      expectAllCompliance([
        evaluateRule(getRuleOrThrow("GOV-SEAL-2"), storedBefore === storedAfter, {
          ...
          evidence: [
            noteEvidence("Compared stored proposal/decision before and after finalize()."),
          ],
        }),
      ]);
  ...
  it(
    ...
    async () => {
      ...
      const threw = await service.finalize(executingProposal, mismatchedCommit, 33).then(
        () => false,
      ...
      expectAllCompliance([
        evaluateRule(getRuleOrThrow("GOV-SEAL-1"), threw, {
          passMessage:
            "finalize() rejects prepared lineage commits whose terminalStatus disagrees with deriveOutcome().",
          failMessage: "finalize() accepted a lineage terminalStatus mismatch.",
          evidence: [
            noteEvidence(
              "Prepared a completed terminal snapshot, then overrode terminalStatus to failed and verified finalize() threw GOV-SEAL-1.",
            ),
          ],
        }),
      ]);
  ...
  it(
    ...
    async () => {
      ...
      const provenanceOk =
        latestAttempt?.proposalRef === governanceCommit.proposal.proposalId &&
        latestAttempt?.decisionRef === governanceCommit.decisionRecord.decisionId;

      expectAllCompliance([
        evaluateRule(getRuleOrThrow("INV-G12"), provenanceOk, {
          passMessage:
            "Governance-active seals carry proposal provenance through SealAttempt.proposalRef/decisionRef.",
          failMessage:
            "Governance seal path lost proposal provenance before persistence in lineage attempts.",
          evidence: [
            noteEvidence(
              "Prepared a governed seal with proposalRef and decisionRef, committed lineage, and read the persisted SealAttempt.",
            ),
          ],
        }),
      ]);
