...
Governance imports **only** the public contract from the projected Lineage SPEC v2.0.0 draft:

| Import | Purpose |
|--------|---------|
| `WorldId`, `BranchId`, `ProvenanceRef` | Identity types |
| `World`, `SealAttempt` | Read-only result / provenance record types |
...
**CRITICAL:** In the projected Core v4 contract, accumulated error-history fields no longer exist. Governance derives failure from `system.lastError` and `pendingRequirements` only.
...
| GOV-OUTCOME-5 | MUST NOT | Governance MUST NOT depend on removed accumulated error-history fields for failure determination |
...
|-------------------|---------------|------|
| `proposal.proposalId` | `SealNextInput.proposalRef` → `PreparedLineageCommit.attempt.proposalRef` → `SealAttempt.proposalRef` | `ProvenanceRef` (opaque string) |
| `decisionRecord.decisionId` | `SealNextInput.decisionRef` → `PreparedLineageCommit.attempt.decisionRef` → `SealAttempt.decisionRef` | `ProvenanceRef` (opaque string) |

Lineage stores these as opaque strings. In the projected v2 model, provenance is attempt-scoped rather than owned by `World` or `WorldEdge`. Governance interprets their meaning.
...
type WorldForkedEvent = BaseGovernanceEvent & {
  ...
  readonly branchId: BranchId;
  readonly forkPoint: WorldId;           // continuity parent (`lineageCommit.edge.from` / `parentWorldId`)
};
```
