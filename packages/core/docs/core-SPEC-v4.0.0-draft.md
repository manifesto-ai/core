...
| v2.0.3 | Expression primitive extensions — string, collection, object, type coercion (15 new kinds) | — |
...
> **Rationale (FDR-011):** Computed dependencies form a DAG — cycles are rejected. Dependencies MUST be topologically sortable. Computed values flow downward, never cycle back. This guarantees: evaluation order is deterministic; no infinite loops in computation; values are always consistent.
> **Canonical statement:** "Computed values flow downward. They never cycle back."
...
> **Rationale (FDR-006):** FlowSpec does NOT include unbounded loops (`while`, `for`, recursion). By limiting expressiveness: all Flows terminate in finite steps; static analysis can verify properties without execution; traces are always finite; resources are bounded. For unbounded iteration, Host controls the loop by repeatedly calling `compute()`.
> **Canonical statement:** "Flows always terminate. Unbounded iteration is Host's responsibility."
...
> **Rationale (FDR-004):** Effects are declarations that something external is needed, not executions. Core declares requirements; Host fulfills them. This preserves purity (no IO inside Core), enables testability (test Flow without real effects), and gives Host flexibility (batch, retry, circuit-break).
> **Canonical statement:** "Core declares requirements. Host fulfills them. Core never executes IO."
...
> **Rationale (FDR-005):** Errors as values means: traceability (errors in Snapshot, visible at any time); locality (error handling = reading Snapshot); serializability (errors survive serialization); explainability (trace shows when/where). Exception-based handling creates non-local control flow, is hard to trace, and difficult to serialize.
> **Canonical statement:** "Errors are values. They live in Snapshot. They never throw."
...
`system.*` fields are not patch targets. Core reports error state through `SystemDelta`, and the Host/Core boundary MUST apply that transition with `applySystemDelta(snapshot, result.systemDelta)`.
...
## Appendix D: FDR Cross-Reference

| FDR | Title | Inlined At |
|-----|-------|------------|
| FDR-001 | Core as Calculator | §3.1 |
| FDR-002 | Snapshot as Only Medium | §13.1 |
| FDR-003 | No Pause/Resume | Appendix C |
| FDR-004 | Effects as Declarations | §10.1 |
| FDR-005 | Errors as Values | §11.1 |
| FDR-006 | Flow is Not Turing-Complete | §8.1 |
| FDR-007 | No Value Binding in Flows | Appendix C |
| FDR-008 | Call Without Arguments | §8.4.5 |
| FDR-009 | Schema-First Design | §4 |
| FDR-010 | Canonical Form and Hashing | §15 |
...
*End of Manifesto Schema Specification*
