...
function normalizeSchemaForHash(schema: SchemaHashInput, mode: SchemaHashMode): SchemaHashInput {
  if (mode === "effective") {
    return schema;
  }
...
function sha256Bytes(data: Uint8Array): Uint8Array {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let i = 16; i < 64; i++) {
      const s0 =
        sha256RotateRight(w[i - 15], 7) ^ sha256RotateRight(w[i - 15], 18) ^ (w[i - 15] >>> 3);
      const s1 =
        sha256RotateRight(w[i - 2], 17) ^ sha256RotateRight(w[i - 2], 19) ^ (w[i - 2] >>> 10);
 ...
 */
export function hashSchemaSync(schema: SchemaHashInput, mode: SchemaHashMode = "semantic"): string {
  const canonical = toCanonical(normalizeSchemaForHash(schema, mode));
  return sha256Sync(canonical);
}

export async function hashSchemaEffective(schema: SchemaHashInput): Promise<string> {
  return hashSchema(schema, "effective");
}

export function hashSchemaEffectiveSync(schema: SchemaHashInput): string {
