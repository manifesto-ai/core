...
import { HOST_CONTEXT, caseTitle, createComplianceSnapshot, pp } from "./helpers.js";

describe("Core CTS snapshot and namespace rules", () => {
  it(
    caseTitle(
      CORE_CTS_CASES.SNAPSHOT_ONTOLOGY,
      "creates canonical state and an empty namespace container",
    ),
    () => {
  ...
  it(
    caseTitle(
      ...
      "applies namespace deltas only under the named namespace root",
    ),
    () => {
      const snapshot = {
  ...
  it(
    caseTitle(
      CORE_CTS_CASES.SNAPSHOT_NAMESPACE_DELTAS,
      "rejects invalid namespace roots, unsafe paths, and invalid merge targets",
    ),
    () => {
      const snapshot = {
        ...createComplianceSnapshot({ count: 1 }),
      ...
      const emptyNamespace = applyNamespaceDeltas(snapshot, [
      ...
      const unsafe = applyNamespaceDeltas(snapshot, [
        {
          namespace: "runtime",
          patches: [
            {
      ...
      const badMerge = applyNamespaceDeltas(snapshot, [
        {
          namespace: "runtime",
          patches: [{ op: "merge", path: pp("request"), value: { requestId: "req-1" } }],
        },
  ...
  it(
    caseTitle(
      CORE_CTS_CASES.SNAPSHOT_NAMESPACE_DELTAS,
      "treats omitted namespace deltas as an empty no-op",
    ),
    () => {
      const snapshot = createComplianceSnapshot({ count: 1 });
