...
describe("Core CTS compute and flow rules", () => {
  it(
    caseTitle(
      ...
      "validates intent input and records failures as values",
    ),
    () => {
  ...
  it(
    caseTitle(
      CORE_CTS_CASES.COMPUTE_ADMISSION,
      "checks availability on initial invocation and skips it on re-entry",
    ),
    () => {
      const schema = createComplianceSchema({
        actions: {
  ...
  it(
    caseTitle(
      ...
      "terminates at effects and records pending requirements in SystemDelta",
    ),
    () => {
      const schema = createComplianceSchema({
        actions: {
              ...
              steps: [
                {
                  kind: "effect",
                  type: "http.get",
                  params: { url: { kind: "lit", value: "/resource" } },
                },
                {
                  kind: "patch",
                  op: "set",
  ...
  it(
    caseTitle(
      CORE_CTS_CASES.FLOW_EFFECT_AND_ERROR,
      "records fail nodes as error values without throwing",
    ),
    () => {
      const schema = createComplianceSchema({
        actions: {
      ...
      expect(() =>
        computeSync(schema, snapshot, createComplianceIntent("failNow"), NEXT_CONTEXT),
      ).not.toThrow();
      const { result, snapshot: finalSnapshot } = computeAndMaterialize(
        schema,
        snapshot,
  ...
  it(
    caseTitle(
      ...
      "resolves dynamic string targets to concrete patches",
    ),
    () => {
      const schema = createComplianceSchema({
        state: {
      ...
      expect(result.patches).toEqual([{ op: "set", path: pp("records.alpha"), value: "A" }]);
  ...
  it(
    caseTitle(
      CORE_CTS_CASES.DYNAMIC_FLOW_PATCH_TARGETS,
      "resolves dynamic integer targets to array index segments",
    ),
    () => {
      const schema = createComplianceSchema({
        actions: {
  ...
  it(
    caseTitle(
      CORE_CTS_CASES.DYNAMIC_FLOW_PATCH_TARGETS,
      "reports invalid dynamic target values as semantic failures",
    ),
    () => {
      const schema = createComplianceSchema({
        state: {
  ...
  for (const { label, value } of [
    ...
    it(
      caseTitle(
        CORE_CTS_CASES.DYNAMIC_FLOW_PATCH_TARGETS,
        `rejects ${label} dynamic target values`,
      ),
      () => {
        const schema = createComplianceSchema({
          state: {
  ...
  it(
    caseTitle(
      CORE_CTS_CASES.DYNAMIC_FLOW_PATCH_TARGETS,
      "rejects unsafe dynamic target values before emitting patches",
    ),
    () => {
      const schema = createComplianceSchema({
        state: {
  ...
  it(
    caseTitle(
      CORE_CTS_CASES.DYNAMIC_FLOW_PATCH_TARGETS,
      "evaluates dynamic targets before patch values",
    ),
    () => {
      const schema = createComplianceSchema({
        state: {
  ...
  it(
    caseTitle(
      CORE_CTS_CASES.DYNAMIC_FLOW_PATCH_TARGETS,
      "reports missing resolved targets and merge target mismatches",
    ),
    () => {
  ...
  it(
    caseTitle(
      CORE_CTS_CASES.DYNAMIC_FLOW_PATCH_TARGETS,
      "uses same-flow working snapshot for later dynamic targets and values",
    ),
    () => {
      const schema = createComplianceSchema({
        state: {
  ...
  it(
    caseTitle(
      CORE_CTS_CASES.DYNAMIC_FLOW_PATCH_TARGETS,
      "keeps runtime allocation deterministic and apply-time patches concrete-only",
    ),
    () => {
      const schema = createComplianceSchema({
        state: {
  ...
  it(
    caseTitle(
      ...
      "materializes Core-owned intent guard namespace deltas during flow evaluation",
    ),
    () => {
      const schema = createComplianceSchema({
        state: {
      ...
      expect(result.status).toBe("complete");
      expect(result.patches).toEqual([{ op: "set", path: pp("observed"), value: "intent-A" }]);
      expect(result.namespaceDelta).toEqual([
        {
          namespace: "core",
          patches: [{ op: "merge", path: pp("causalGuards"), value: { guardA: "intent-A" } }],
        },
      ]);
  ...
  it(
    caseTitle(
      CORE_CTS_CASES.SNAPSHOT_NAMESPACE_DELTAS,
      "skips Core intent guard bodies already marked for the current intent",
    ),
    () => {
      const schema = createComplianceSchema({
        state: {
  ...
  it(
    caseTitle(
      CORE_CTS_CASES.SNAPSHOT_NAMESPACE_DELTAS,
      "rejects namespacePatch flow nodes from Core schemas",
    ),
    () => {
      const schema = createComplianceSchema({
        actions: {
