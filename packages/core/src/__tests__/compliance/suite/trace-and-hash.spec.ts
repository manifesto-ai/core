...
describe("Core CTS trace and hash rules", () => {
  it(
    caseTitle(
      ...
      "emits complete trace graph metadata for compute results",
    ),
    () => {
      ...
      expect(Object.keys(result.trace.nodes).sort()).toEqual(
        collectTraceNodes(result.trace.root).sort(),
      );
  ...
  it(
    caseTitle(
      CORE_CTS_CASES.TRACE_AND_HASH,
      "keeps namespace trace node kinds distinct from domain patch traces",
    ),
    () => {
  ...
  it(
    caseTitle(
      CORE_CTS_CASES.TRACE_AND_HASH,
      "computes deterministic schema hashes over canonical form",
    ),
    () => {
      ...
      expect(hashSchemaSync(schemaWithoutHash)).toBe(
        hashSchemaSync({
