...
describe("Core CTS patch and system delta rules", () => {
  it(
    caseTitle(
      ...
      "roots domain patches at snapshot.state and not platform roots",
    ),
    () => {
  ...
  it(
    caseTitle(
      CORE_CTS_CASES.PATCH_DOMAIN_ROOT,
      "rejects namespace-looking patches when no state field declares that root",
    ),
    () => {
      const schema = createComplianceSchema();
  ...
  it(
    caseTitle(
      CORE_CTS_CASES.PATCH_DOMAIN_ROOT,
      "rejects invalid patch values as runtime validation failures",
    ),
    () => {
      const schema = createComplianceSchema();
      const snapshot = createComplianceSnapshot({ count: 1 }, schema.hash);
  ...
  it(
    caseTitle(
      ...
      "applies system transitions through SystemDelta and preserves domain state",
    ),
    () => {
