...
import { getAvailableActions, isActionAvailable, isIntentDispatchable } from "../../../index.js";
...
describe("Core CTS availability and dispatchability queries", () => {
  it(
    caseTitle(
      ...
      "evaluates action availability as a pure current-snapshot query",
    ),
    () => {
      ...
      expect(() => isActionAvailable(schema, snapshot, "missing")).toThrow(
      ...
      expect(() =>
        isActionAvailable(invalidSchema, invalidSnapshot, "invalidAvailability"),
      ).toThrow("Availability condition must return boolean");
    },
  );
  ...
  it(
    caseTitle(
      CORE_CTS_CASES.ACTION_QUERIES,
      "evaluates dispatchability against bound intent input and short-circuits unavailable actions",
    ),
    () => {
      const schema = createComplianceSchema({
        actions: {
      ...
      expect(
        isIntentDispatchable(schema, snapshot, createComplianceIntent("withdraw", { amount: 5 })),
      ).toBe(true);
      expect(
        isIntentDispatchable(schema, snapshot, createComplianceIntent("withdraw", { amount: 15 })),
      ).toBe(false);
      expect(
        isIntentDispatchable(schema, snapshot, createComplianceIntent("unavailableInvalid")),
      ).toBe(false);
      expect(() =>
        isIntentDispatchable(schema, snapshot, createComplianceIntent("missing")),
      ).toThrow("Unknown action: missing");
      expect(() =>
        isIntentDispatchable(schema, snapshot, createComplianceIntent("invalidDispatchability")),
