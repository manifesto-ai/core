...
import type { LabState, LabStatus } from "../types.js";
  ...
  switch (event.type) {
      ...
      return {
        ...
        currentStep:
          currentState.status === "running"
            ...
            : 1,
        pendingHITL:
          currentState.status === "running" ? currentState.pendingHITL : [],
      } as LabState;
    ...
    case "proposal:decided":
      if (event.decision === "pending") {
        ...
        return {
          status: "waiting_hitl",
          proposal: {} as Proposal, // Will be filled by HITL controller
 ...
 */
export function abortLabState(reason: string): LabState {
  return {
    status: "aborted",
    reason,
  };
 ...
 */
export function addPendingHITL(
  currentState: LabState,
  proposal: Proposal
): LabState {
...
export function resolvePendingHITL(
  ...
  return {
    ...currentState,
    pendingHITL: currentState.pendingHITL.filter(
