 ...
 */
export function buildPrompt(options: BuildPromptOptions): HITLPrompt {
  const {
    snapshot,
    proposal,
    pendingReason,
  ...
  const stateRenderer: SnapshotRenderer =
    promptOptions.stateRenderer ?? defaultSnapshotRenderer;
  ...
  const promptOptions2: HITLPrompt["options"] = [];
 ...
 * Build situation description.
 */
function buildSituation(context: RenderContext, proposal: Proposal): string {
  ...
  return (
    `You are operating at Necessity Level ${context.level} (${levelDesc}). ` +
 ...
 */
function actionToOption(action: HITLAction): HITLPrompt["options"][0] {
  switch (action.type) {
    case "retry":
      return {
        id: `retry-${Date.now()}`,
 ...
 */
export function promptToText(prompt: HITLPrompt): string {
  const lines: string[] = [];
 ...
 * @returns JSON string
 */
export function promptToJSON(prompt: HITLPrompt, pretty = false): string {
