 ...
 */
export const defaultHeaderRenderer: HeaderRenderer = (ctx) => {
 ...
 */
export const defaultFooterRenderer: FooterRenderer = (ctx) => {
 ...
 */
export const defaultSnapshotRenderer: SnapshotRenderer = (snapshot, ctx) => {
 ...
 */
export const defaultActionRenderer: ActionRenderer = (
  intent,
  before,
  after,
  ctx
  ...
  const body = (intent as { body?: { type?: string } }).body;
 ...
 */
export const defaultProposalRenderer: ProposalRenderer = (proposal, ctx) => {
 ...
 */
export const defaultReasoningRenderer: ReasoningRenderer = (
  reasoning,
  confidence,
  ctx
) => {
 ...
 */
export const defaultLayoutRenderer: LayoutRenderer = (sections) => {
...
export function mergeRenderers(
  custom?: ProjectionComponents
): Required<ProjectionComponents> {
  return {
...
export function renderAllSections(
  components: Required<ProjectionComponents>,
  options: {
  ...
  const actions =
    options.intent && options.beforeSnapshot && options.afterSnapshot
      ? components.renderAction(
          options.intent,
  ...
  const reasoning =
    options.reasoning !== null && options.reasoning !== undefined
      ? components.renderReasoning(
          options.reasoning,
...
export function renderComplete(
  components: Required<ProjectionComponents>,
  options: Parameters<typeof renderAllSections>[1]
): string {
