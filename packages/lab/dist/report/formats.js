 ...
 */
export function enhanceReport(report) {
    return {
        ...report,
        toMarkdown: () => toMarkdown(report),
        toMarkdownFile: (path) => toMarkdownFile(report, path),
        toHTML: () => toHTML(report),
        toHTMLFile: (path) => toHTMLFile(report, path),
 ...
 */
export async function toMarkdownFile(report, path) {
...
export function toHTML(report) {
    const outcomeClass = report.outcome === "success" ? "success" : report.outcome === "failure" ? "failure" : "aborted";
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
 ...
 * Save HTML to file.
 */
export async function toHTMLFile(report, path) {
