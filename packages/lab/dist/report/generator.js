 ...
 */
export function generateReport(trace, options, startedAt = Date.now()) {
...
function determineOutcome(events) {
    ...
    const hasFailure = events.some((e) => e.type === "termination" && e.outcome === "failure");
    return hasFailure ? "failure" : "success";
}
/**
 * Extract failure explanation from trace events.
 */
function extractFailureExplanation(events) {
    const explanationEvent = events.find((e) => e.type === "failure.explanation");
