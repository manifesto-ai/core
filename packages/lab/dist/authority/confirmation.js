 ...
 */
export function createConfirmationAuthority(options) {
    return {
        level: 3,
        ...
        async evaluate(proposal, _binding) {
            ...
            const ambiguities = grounding.ambiguities;
            const unresolvedAmbiguities = ambiguities?.filter((a) => a.resolutionMethod === "unresolved");
            ...
            const resolutions = grounding.referenceResolutions;
            const lowConfidenceResolutions = resolutions?.filter((r) => r.confidence < 0.8);
