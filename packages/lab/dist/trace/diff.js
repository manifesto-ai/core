...
export function diffTraces(traceA, traceB) {
    ...
    const identical = divergencePoint === null &&
    ...
    const cause = divergencePoint !== null
 ...
 * Build event-by-event comparison.
 */
function buildEventDiffs(eventsA, eventsB) {
 ...
 */
function findDivergencePoint(diffs) {
 ...
 */
function inferDivergenceCause(eventA, eventB, diffs, divergenceSeq) {
 ...
 */
function inferSameTypeCause(eventA, eventB) {
    switch (eventA.type) {
        case "authority.decision":
            if (eventA.type === eventB.type &&
...
export function formatDiff(diff) {
    ...
    const differentCount = diff.eventDiffs.filter((d) => d.status !== "identical").length;
    const identicalCount = diff.eventDiffs.filter((d) => d.status === "identical").length;
    ...
    for (const d of differences.slice(0, 5)) {
        ...
        if (d.eventA)
            lines.push(`    A: ${d.eventA.type}`);
        if (d.eventB)
            lines.push(`    B: ${d.eventB.type}`);
        if (d.differences) {
 ...
 */
export function areTracesIdentical(traceA, traceB) {
