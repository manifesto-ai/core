    ...
    try {
        ...
        for (let i = 0; i < trace.events.length; i++) {
            ...
            if (originalEvent.type === "proposal") {
                await replayProposal(originalEvent, world, actorOverride);
                eventsReplayed++;
 ...
 */
async function replayProposal(event, world, actorOverride) {
 ...
 */
function mapWorldEventToReplayEvent(event, seq) {
 ...
 */
function checkDivergence(original, replay) {
    // Type mismatch
 ...
 */
function determineReplayOutcome(events, divergences, mode) {
 ...
 */
export async function replayPartial(trace, upToSeq, options) {
 ...
 */
export async function findFirstDivergence(trace, options) {
