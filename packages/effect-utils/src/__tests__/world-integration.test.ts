...
import type { EffectContext, Requirement } from "@manifesto-ai/host";
...
function applyPatches(snapshot: Snapshot, patches: Patch[]): Snapshot {
  ...
  for (const patch of patches) {
    if (patch.op === "set") {
      ...
      if (parts[0] === "data" && parts.length > 1) {
        ...
        (newSystem as Record<string, unknown>)[parts[1]] = patch.value;
      }
    }
  }
  ...
  return {
    ...snapshot,
    data: newData,
    system: newSystem as Snapshot["system"],
 ...
 */
function createMockHostWithHandlers(
  handlers: Map<string, EffectHandler>,
  initialSnapshot?: Snapshot
): { host: HostInterface; getSnapshot: () => Snapshot } {
  ...
  const host: HostInterface = {
    dispatch: vi.fn().mockImplementation(async (intent) => {
      ...
      if (handler) {
        try {
          const context = createMockContext(currentSnapshot, intent.input as Record<string, unknown>);
          const patches = await handler(effectType, intent.input as Record<string, unknown>, context);
          currentSnapshot = applyPatches(currentSnapshot, patches);
          return { status: "complete" as const, snapshot: currentSnapshot };
        } catch (error) {
          return {
            ...
            snapshot: applyPatches(currentSnapshot, [
              {
                ...
                value: { $error: true, message: (error as Error).message },
              },
            ]),
          };
        }
...
describe("World + Effect-Utils Integration", () => {
  describe("Effect Handler Registration", () => {
    it("should execute effect-utils handler through World → Host pipeline", async () => {
      ...
      // 6. Register actor
      world.registerActor(
        { actorId: "test-user", kind: "human", name: "Test User" },
        { mode: "auto_approve" }
      );
    ...
    it("should handle effect-utils implementation errors gracefully", async () => {
      ...
      world.registerActor(
        { actorId: "test-user", kind: "human", name: "Test" },
        { mode: "auto_approve" }
      );
  ...
  describe("Parallel Aggregation Pattern", () => {
    it("should aggregate multiple API calls with partial failure", async () => {
      ...
      const aggregateHandler = createHandler(aggregateSchema, async (input) => {
        ...
        return {
          ais: (fulfilled.ais as { vessel: string } | undefined) ?? null,
          weather: (fulfilled.weather as { temp: number } | undefined) ?? null,
          errors,
        };
      });
      ...
      world.registerActor(
        { actorId: "system", kind: "system", name: "System" },
        { mode: "auto_approve" }
      );
  ...
  describe("Resilient Fetch with Combinators", () => {
    it("should retry and succeed on third attempt", async () => {
      ...
      const handler = createHandler(schema, async () => {
        type ResultType = { data: string; fromCache: boolean };

        const resilientFetch = withFallback<ResultType>(
          withRetry(mockFetch, { maxRetries: 3, baseDelay: 0, backoff: "none" }),
          mockCache
        );
      ...
      world.registerActor(
        { actorId: "app", kind: "system", name: "App" },
        { mode: "auto_approve" }
      );
    ...
    it("should fallback when all retries exhausted", async () => {
      ...
      const handler = createHandler(schema, async () => {
        ...
        const resilientFetch = withFallback<ResultType>(
          withRetry(mockFetch, { maxRetries: 2, baseDelay: 0, backoff: "none" }),
          mockCache
        );
  ...
  describe("Authority Integration", () => {
    it("should execute effect after HITL approval", async () => {
      ...
      const owner = { actorId: "owner", kind: "human" as const, name: "Owner" };
      const agent = { actorId: "agent", kind: "agent" as const, name: "Agent" };
      ...
      // Owner approves
      const approveResult = await world.processHITLDecision(
        submitResult.proposal.proposalId,
        "approved",
        "OK"
      );
    ...
    it("should not execute effect on HITL rejection", async () => {
      ...
      // Owner rejects
      const rejectResult = await world.processHITLDecision(
        submitResult.proposal.proposalId,
        "rejected",
        "Denied"
      );
    ...
    it("should auto-approve based on policy rules", async () => {
      ...
      world.registerActor(
        { actorId: "bot", kind: "agent", name: "Bot" },
        {
          mode: "policy_rules",
          rules: [
  ...
  describe("Sequential Effect Execution", () => {
    it("should execute effect-utils handlers across multiple sequential proposals", async () => {
      ...
      let callCount = 0;
      const mockApi = vi.fn().mockImplementation(async (userId: string) => ({
        id: userId,
        name: `User ${++callCount}`,
      }));
      ...
      world.registerActor(
        { actorId: "user", kind: "human", name: "User" },
        { mode: "auto_approve" }
      );
