...
describe("Integration: Full Handler Flow", () => {
  ...
  it("should create handler that returns valid patches on success", async () => {
    ...
    const patches = await handler(
      "api.user.fetch",
      { userId: "user-123", includeProfile: true },
      createMockContext()
    );
...
describe("Integration: Resilient API Call Pattern (SPEC §9.1)", () => {
  ...
  it("should implement resilient fetch with retry, timeout, and fallback", async () => {
    type DataResult = { data: string; fromCache: boolean };
    let callCount = 0;
    const mockFetch = async (): Promise<DataResult> => {
      callCount++;
      if (callCount < 3) {
    ...
    const mockCache: DataResult = { data: "cached data", fromCache: true };

    const resilientFetch = withFallback<DataResult>(
      withRetry(withTimeout(mockFetch, 5000), {
        maxRetries: 2,
        backoff: "exponential",
        baseDelay: 100,
      }),
...
describe("Integration: Error Types", () => {
  ...
  it("should produce TimeoutError with correct properties", async () => {
    ...
    expect((caughtError as TimeoutError).ms).toBe(100);
    expect((caughtError as TimeoutError).code).toBe("TIMEOUT_ERROR");
  });
  ...
  it("should produce RetryError with attempt count", async () => {
    ...
    try {
      ...
      expect((e as RetryError).attempts).toBe(4); // 1 initial + 3 retries
      expect((e as RetryError).code).toBe("RETRY_EXHAUSTED");
    }
  });
});
