# Manifesto World Protocol Specification

> **Status:** Legacy Monolith Reference (Living Document)
...
| v2.0.2 | Host-World Data Contract — `$host` namespace, deterministic hashing, baseSnapshot | FDR-W032 ~ W033 |
| v2.0.3 | Platform Namespace Extension — `$mel` namespace, unified platform namespace policy | FDR-W034 ~ W035 |
| v2.0.4 | Future-proof Platform Namespace — `$`-prefix pattern for automatic handling | — |
| v2.0.5 | Head Query API — Head definition (branch pointer), resume contract, branch persistence | FDR-W036 ~ W038 |
...
- [Appendix B: Cross-Reference](#appendix-b-cross-reference)
- [Appendix C: Migration from v1.0](#appendix-c-migration-from-v10)
...
### 4.2 World's "Does NOT Know" Boundary

> **Rationale (FDR-W029):** Ignorance is architecture. When World doesn't know Host internals: Host can evolve without breaking World; World cannot accidentally couple to Host; testing World doesn't require Host; World's concerns remain purely governance. This is stronger than "should not use" — it's "cannot even reference."
...
### 6.7 Epoch-based Ingress Cancellation
...
### 7.4 Terminal Snapshot Validity
...
### 7.7 Context Determinism Alignment
...
> **Rationale (FDR-W032):** Host MUST store internal execution state in `snapshot.data.$host`. World MUST exclude this namespace from snapshotHash. Same semantic state must produce same WorldId regardless of intent slot state. The `$` prefix signals "reserved" namespace, unlikely to collide with domain schemas.
> **Why not `system.$host`?** `system.*` is Core-owned vocabulary (status, errors, pendingRequirements). Host should not pollute Core's namespace.
> **Alternatives rejected:** Store in `system.$host` (pollutes Core namespace); separate channel (violates Snapshot-as-sole-truth); hash everything (intent slot changes create spurious Worlds); informal convention (implicit contracts lead to bugs).

| Concern | Solution |
|---------|----------|
| Host needs persistent execution context | Store in `data.$host` |
| Compiler needs guard continuity | Store in `data.$mel` |
| World hash must be deterministic | Exclude all `$`-prefixed namespaces from hash |
 ...
 *
 * Per Core SPEC SCHEMA-RESERVED-1 and World SPEC §7.9.1:
...
> **Rationale (FDR-W027):** World owns governance events; App owns execution telemetry events. "Results are World's; Process is App's." Execution telemetry (compute iterations, patches, effects) is operational observation, not governance. By separating ownership: World doesn't need TraceEvent structure; Host can evolve without breaking World; App absorbs execution model changes.
> **Alternatives rejected:** Keep all events in World (couples World to Host internals); create Runtime layer (unnecessary, App already knows both); remove telemetry (operational visibility is valuable).
...
### 8.4 Non-Interference Constraints
...
### 8.5 Scheduled Reaction Execution
...
### 9.2 Snapshot Storage Clarification
...
> **Rationale (FDR-W036):** Head is defined as a World referenced by `Branch.head` (pointer semantics), not as a lineage DAG leaf node. The leaf definition causes two critical conflicts: (1) failed Worlds become leaves but branch heads don't advance (BRANCH-7), making "resume from leaf" return failed state; (2) fork heads disappear when parent gains children, breaking Active Horizon.
> **Alternatives rejected:** Head as lineage leaf (breaks BRANCH-7 and Active Horizon); App tracks head separately (duplicates World's responsibility); WorldStore.getHeads() made public (violates ADR-003 encapsulation).
> **Consequences:** HEAD-1~8 rules; `WorldHead` includes `branchId`; `createdAt` = head World's creation time; `schemaHash` = Branch's current schemaHash.
...
| HEAD-8 | MUST | `WorldHead.schemaHash` MUST be the Branch's current schemaHash |

> **Rationale (FDR-W037):** `getLatestHead()` returns the most recent Head by `createdAt`, with no completed/failed preference logic. BRANCH-7 already guarantees all branch heads are completed — adding preference logic creates a MUST/SHOULD normative conflict and is operationally unnecessary. The invariant is structural (governance layer), not algorithmic (query layer).
> **Consequences:** HEAD-4 is simple `max(createdAt)` — no filtering needed; normatively clean with all MUST rules.

#### 9.7.2 Head Query API

Head query is part of World's read-only query contract (ADR-003 §5). Implementation resides in AppRuntime using BranchManager + WorldStore.

```typescript
// Added query APIs (implemented in AppRuntime, delegated via App)
app.getHeads(): Promise<WorldHead[]>;
app.getLatestHead(): Promise<WorldHead | null>;
```

**`getHeads()`** returns all current Heads (one per Branch), ordered by `createdAt` descending, tie-broken by `worldId` ascending, then `branchId` ascending.

**`getLatestHead()`** returns the Head with the most recent `createdAt`. Returns null only if no Branches exist.
...
Branch head: W1   ← BRANCH-7: head doesn't advance on failure
Lineage leaf: W2  ← Only node with no children
Resume from leaf → failed state → UX broken
Resume from pointer → last good state → Correct
```

**Conflict B — Fork head disappears:**
```
main:       W1 → W2 (completed)
experiment: → W1 (fork point)
Leaf: Only W2 (W1 has child)
Pointer: main→W2, experiment→W1 — both represented
```

### 9.8 Resume Semantics

Resume is the process of reconstructing App runtime state from persisted World lineage after a restart.

> **Rationale (FDR-W038):** Resume is implemented as composition of World's existing query primitives, not as a new App configuration option. Default resume uses persisted active branch head (preserving user's branch context), NOT `getLatestHead()`. `getLatestHead()` is positioned as a query primitive for UI ("most recently updated branch") and alternative resume strategies.
> **Alternatives rejected:** App configuration option `resume: { mode: 'latest' }` (hides logic, conflicts with "App is thin facade" ADR-004); dedicated Resume API (unnecessary when primitives suffice).
> **Consequences:** World adds `getHeads()` + `getLatestHead()` as query primitives; App delegates; default resume reads persisted active branch.
...
| RESUME-5 | MAY | App MAY implement alternative resume strategies |
...
#### 9.8.2 Schema Migration on Resume
...
#### 9.9.1 Branch Persistence Rules
...
| BRANCH-PERSIST-4 | SHOULD | Branch state update SHOULD be atomic with World creation |
| BRANCH-PERSIST-5 | SHOULD | Branch state SHOULD be stored alongside WorldStore index |

#### 9.9.2 Crash Recovery

| Rule ID | Level | Description |
|---------|-------|-------------|
| BRANCH-RECOVER-1 | MUST | App MUST validate that each persisted branch head WorldId exists in WorldStore |
| BRANCH-RECOVER-2 | MUST | Branches with invalid head WorldIds MUST be excluded from resume and a warning MUST be logged |
| BRANCH-RECOVER-3 | MUST | If all persisted branches have invalid heads, App MUST fall back to fresh start |
...
### 10.7 Layer Boundary Invariants
...
### B.6 World FDR v2.0.2/v2.0.3 Alignment

| FDR | SPEC Section | Key Rules |
|-----|--------------|-----------|
| W032 | §7.9 | HOST-DATA-* |
| W033 | §5.4, §7.3 | TERM-UNIFIED-* |
| W034 | §5.5 | WORLD-HASH-10 |
| W035 | §7.1 | WORLD-HEXEC-7, HEXEC-ABORT-* |

### B.7 World FDR v2.0.5 Alignment

| FDR | SPEC Section | Key Rules |
|-----|--------------|-----------|
| W036 | §9.7 | HEAD-1~8 |
| W037 | §9.7 | HEAD-4 (no preference logic) |
| W038 | §9.8 | RESUME-1~6 |
...
### C.6 New Requirements (v2.0.4)

| Requirement | Section |
|-------------|---------|
| `stripPlatformNamespaces()` uses `$`-prefix pattern | §7.9.4 |
| `isPlatformNamespace()` helper | §7.9.4 |
| `PLATFORM_NAMESPACE_PREFIX` constant | §7.9.4 |

### C.7 New Requirements (v2.0.5)

| Requirement | Section |
|-------------|---------|
| Head definition (branch pointer) | §9.7 |
| HEAD-1~8 rules | §9.7.1 |
| Head Query API (`getHeads()`, `getLatestHead()`) | §9.7.2 |
| Resume semantics | §9.8 |
| RESUME-1~6 rules | §9.8.1 |
| Schema migration on resume | §9.8.2 |
| Branch persistence | §9.9 |
| BRANCH-PERSIST-1~5 rules | §9.9.1 |
| Crash recovery (BRANCH-RECOVER-1~3) | §9.9.2 |
| INV-W16~W19 | §10.3 |

---

*End of Manifesto World Protocol Specification*
