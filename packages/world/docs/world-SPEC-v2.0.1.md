# Manifesto World Protocol Specification v2.0.1
...
This document defines the **Manifesto World Protocol v2.0.1**.
...
**v2.0.1 Focus**: ADR-001 Layer Separation—clarifying that World owns **governance results**, while App owns **execution telemetry**.
...
| Host v2.0.1 integration | ExecutionKey, canonical head, terminal snapshot |
...
### 4.1 Constitutional Boundary (v2.0.1)
...
### 4.2 World's "Does NOT Know" Boundary (v2.0.1)
...
function deriveTerminalStatusForHash(
  ...
  // Current error state → 'error' (lastError is non-null)
  if (snapshot.system.lastError != null) {
    return 'error';
  }
  // Non-empty pending → 'error' (WORLD-TERM-1 violation)
...
- `system.lastError` represents current error state (null when no active error).
- To ensure WorldId consistency, we normalize to exactly two values: `completed` or `error`.
...
| WORLD-TERM-5 | `terminalStatus` for hash MUST be exactly `'completed'` or `'error'` |
...
### 7.1 HostExecutor Interface (v2.0.1)
...
#### 8.8.3 Execution Result Events (v2.0.1)
...
### 9.2 Snapshot Storage Clarification (v2.0.1)
...
### 9.3 BaseSnapshot Restoration (v2.0.1)

When executing a new Proposal, HostExecutor needs a `baseSnapshot` to start from.
This section clarifies restoration strategies for App implementations.

**App's Responsibility:**
- App MUST provide `baseSnapshot` to HostExecutor (per §7.1, §4.4 diagram)
- App MAY choose any restoration strategy below
- World does NOT store or manage baseSnapshot restoration
...
| Rule ID | Description |
|---------|-------------|
| PERSIST-BASE-1 | App MUST be able to provide baseSnapshot for any valid baseWorld |
| PERSIST-BASE-2 | Restoration strategy is App's choice (not World's concern) |
...
### 10.7 Layer Boundary Invariants (v2.0.1)
...
An implementation claiming compliance with **Manifesto World Protocol v2.0.1** MUST:
...
22. **[v2.0.1]** Only define governance events (WORLD-EVT-OWN-*)
23. **[v2.0.1]** NOT depend on Host package (WORLD-BOUNDARY-*)
24. **[v2.0.1]** Define but NOT implement HostExecutor (WORLD-HEXEC-*)
...
| terminalStatus normalization | Hash uses 'completed'\|'error' only |
...
| Component | Change Required (v2.0) | Change Required (v2.0.1) |
|-----------|------------------------|--------------------------|
...
*End of Manifesto World Protocol Specification v2.0.1*
