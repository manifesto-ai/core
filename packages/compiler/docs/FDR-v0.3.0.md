...
11. [Compiler Lowering (v0.3.0)](#11-compiler-lowering-v030)
...
A23. Same $system.<key> in same action = same value. [v0.3.0]
A24. Replay uses Snapshot only, no separate trace. [v0.3.0]
...
### 10.7 Replay Guarantee

```
REPLAY MODEL:

  System values are patched into Snapshot.
  Snapshot IS the complete truth.
  Replay = provide same Snapshot → identical output.

GUARANTEE:
  compute(Snapshot₁, Intent) at T₁ = compute(Snapshot₁, Intent) at T₂
  
  Because:
  - System value slots in Snapshot already populated
  - Guards see isNull(slot) = false
  - Effects are SKIPPED
...
FOR each action in domain:
  ...
  2. FOR each unique key found:
     a. ALLOCATE state slot: _sys$<action>$<key-normalized>
     b. ALLOCATE marker slot: _sys$<action>$<key-normalized>$marker
     c. GENERATE guarded effect at action start:
        once(<marker>) when isNull(<slot>) {
          patch <marker> = $meta.intentId
          effect system.get({ key: "<key>", into: <slot> })
        }
  
  3. REWRITE all $system.<key> references → <slot>
  
  4. MODIFY original guard conditions:
     - Add isNotNull(<slot>) for each system value used
     - Ensures original logic waits for all values
```

### 11.2 State Slot Naming

```
NAMING CONVENTION:

State slot: _sys$<action>$<key-normalized>
Marker slot: _sys$<action>$<key-normalized>$marker
...
Examples:
  $system.uuid in addTask → _sys$addTask$uuid
  $system.time.now in addTask → _sys$addTask$time_now
```

### 11.3 Complete Lowering Example
...
domain TaskManager {
  state {
    ...
    // Compiler-generated system value slots
    _sys$addTask$uuid: string | null = null
    _sys$addTask$uuid$marker: string | null = null
    _sys$addTask$time_now: number | null = null
    _sys$addTask$time_now$marker: string | null = null
  }
  
  action addTask(title: string) {
    // Phase 1: Acquire $system.uuid
    once(_sys$addTask$uuid$marker) when isNull(_sys$addTask$uuid) {
      patch _sys$addTask$uuid$marker = $meta.intentId
      effect system.get({ key: "uuid", into: _sys$addTask$uuid })
    }
    
    // Phase 2: Acquire $system.time.now (after uuid ready)
    once(_sys$addTask$time_now$marker) 
      when and(isNotNull(_sys$addTask$uuid), isNull(_sys$addTask$time_now)) {
      patch _sys$addTask$time_now$marker = $meta.intentId
      effect system.get({ key: "time.now", into: _sys$addTask$time_now })
    }
    
    // Phase 3: Original logic (rewritten, depends on all system values)
    once(creating) 
      when and(
        isNotNull(_sys$addTask$uuid),
        isNotNull(_sys$addTask$time_now),
        neq(trim(title), "")
      ) {
      patch creating = $meta.intentId
      patch tasks[_sys$addTask$uuid] = {
        id: _sys$addTask$uuid,
        title: title,
        createdAt: _sys$addTask$time_now
      }
    }
  }
}
...
### 11.4 Guard Insertion Rules

**All system effects follow v0.2.5 once() rules (FDR-MEL-044):**

```
1. System effects MUST be inside once() guard
2. Marker patch MUST be first statement
3. Condition includes isNull(<target>) to prevent re-acquisition
4. Later effects depend on earlier effects being populated
5. Original user guards get dependency conditions added
```

### 11.5 Dependency Ordering

When multiple `$system.*` values are used:
...
Example:
  ...
  First occurrence: $system.uuid (in tasks[...])
  Second occurrence: $system.time.now (in createdAt)
  
  Pipeline:
    effect uuid → effect time.now → original logic
    
  Dependencies:
    - uuid effect: when isNull(_sys$uuid)
    - time.now effect: when and(isNotNull(_sys$uuid), isNull(_sys$time_now))
    - original: when and(isNotNull(_sys$uuid), isNotNull(_sys$time_now), ...)
```
...
{ kind: 'get', path: [{ kind: 'prop', name: '_sys$addTask$uuid' }] }
```
...
### 12.4 Effect Examples with $item
...
### 12.5 Composition Effects (flatMap, groupBy)
...
### 13.1 From MEL v0.2.x to v0.3.0
...
### 13.2 From MEL v0.2.1 to v0.2.2
...
MEL v0.3.0 is designed with the following AI-Native principles:
