...
export const COMPILER_COMPLIANCE_CASES: readonly CompilerComplianceCase[] = [
  complianceCase(
    CCTS_CASES.GRAMMAR_RESERVED_IDENTIFIERS,
    "grammar",
    "Reserved namespaces and reserved user identifiers are rejected.",
  ),
  complianceCase(
    CCTS_CASES.GRAMMAR_CANONICAL_SURFACE,
    "grammar",
    "Non-canonical surface syntax is rejected.",
  ),
  complianceCase(
    CCTS_CASES.GRAMMAR_INVALID_SYSTEM_REF,
    "grammar",
    "Invalid system references are diagnosed.",
  ),
  complianceCase(
    CCTS_CASES.GRAMMAR_ONCE_INTENT_CONTEXTUAL,
    "grammar",
    "onceIntent is parsed contextually.",
  ),
  complianceCase(
    CCTS_CASES.GRAMMAR_OBJECT_SPREAD_BOUNDARY,
    "grammar",
    "Current object-spread surface and adjacent JS-form boundaries are enforced in compliance.",
  ),

  complianceCase(
    CCTS_CASES.ANNOTATIONS_SURFACE,
    "annotations",
    "@meta target placement, sibling sidecar target-key shape, and tooling sidecar emission determinism are enforced.",
  ),
  complianceCase(
    CCTS_CASES.ANNOTATIONS_PAYLOAD,
    "annotations",
    "@meta payload literal-only and depth constraints are enforced.",
  ),
  complianceCase(
    CCTS_CASES.ANNOTATIONS_INVARIANTS,
    "annotations",
    "Tooling sidecar semantic erasure invariants are enforced.",
  ),
  complianceCase(
    CCTS_CASES.ANNOTATIONS_RUNTIME_BOUNDARY,
    "annotations",
    "Tooling-only DomainModule runtime-boundary guards are enforced.",
  ),
  complianceCase(
    CCTS_CASES.ANNOTATIONS_DIAGNOSTICS,
    "annotations",
    "Annotation and source-map diagnostic families are enforced.",
  ),

  complianceCase(
    CCTS_CASES.CONTEXT_COMPUTED_SYSTEM,
    "context",
    "$system is rejected in computed expressions.",
  ),
  complianceCase(
    CCTS_CASES.CONTEXT_STATE_INIT_SYSTEM,
    "context",
    "$system is rejected in state initializers.",
  ),
  complianceCase(
    CCTS_CASES.CONTEXT_AVAILABLE_PURITY,
    "context",
    "available when stays schema-pure.",
  ),

  complianceCase(
    CCTS_CASES.STATE_INLINE_OBJECTS,
    "state-and-computed",
    "Inline object types in schema positions are tracked.",
  ),
  complianceCase(
    CCTS_CASES.STATE_ACTION_INPUT_FIELDS,
    "state-and-computed",
    "Action inputs lower to FieldSpec object fields.",
  ),
  complianceCase(
    CCTS_CASES.STATE_ACTION_INPUT_OMISSION,
    "state-and-computed",
    "Actions without params omit ActionSpec.input.",
  ),
  complianceCase(
    CCTS_CASES.STATE_CONSTANT_DEFAULTS,
    "state-and-computed",
    "Constant state initializers lower to concrete defaults.",
  ),
  complianceCase(
    CCTS_CASES.STATE_COMPUTED_DEPS,
    "state-and-computed",
    "Computed deps are extracted and ordered.",
  ),
  complianceCase(
    CCTS_CASES.STATE_SCHEMA_REFS,
    "state-and-computed",
    "Schema-position references and computed cycles are diagnosed.",
  ),
  complianceCase(
    CCTS_CASES.STATE_UNSUPPORTED_TYPES,
    "state-and-computed",
    "Unsupported FieldSpec-lowering types are tracked.",
  ),
  complianceCase(
    CCTS_CASES.STATE_PATCH_MERGE,
    "state-and-computed",
    "patch merge remains a shallow partial-object operation.",
  ),
  complianceCase(
    CCTS_CASES.STATE_COALESCE_NARROWING,
    "state-and-computed",
    "coalesce narrows compatible nullable branches for downstream typing.",
  ),
  complianceCase(
    CCTS_CASES.STATE_VALUES_RECORD_TYPING,
    "state-and-computed",
    "values(Record<string, T>) preserves typed collection flow semantics.",
  ),
  complianceCase(
    CCTS_CASES.STATE_ARG_SELECTION_COVERAGE,
    "state-and-computed",
    "argmax/argmin keep nullable labels only when candidate eligibility is not exhaustively covered.",
  ),
  complianceCase(
    CCTS_CASES.STATE_OBJECT_SPREAD_TYPING,
    "state-and-computed",
    "Current object-spread typing and patch assignability are enforced.",
  ),
  complianceCase(
    CCTS_CASES.STATE_OBJECT_SPREAD_CONSUME,
    "state-and-computed",
    "Current optional spread-field consumption rules are enforced.",
  ),

  complianceCase(
    CCTS_CASES.ACTIONS_GUARDED_BODY,
    "actions-and-control",
    "Action mutations remain guarded.",
  ),
  complianceCase(
    CCTS_CASES.ACTIONS_ONCE_DESUGARING,
    "actions-and-control",
    "once() desugars to intent-guarded marker writes.",
  ),
  complianceCase(
    CCTS_CASES.ACTIONS_ONCE_INTENT_DESUGARING,
    "actions-and-control",
    "onceIntent lowers to Core intent guards.",
  ),
  complianceCase(
    CCTS_CASES.ACTIONS_FAIL_STOP_LOWERING,
    "actions-and-control",
    "fail/stop lower to flow-control nodes.",
  ),
  complianceCase(
    CCTS_CASES.ACTIONS_FAIL_STOP_DIAGNOSTICS,
    "actions-and-control",
    "Guard requirements for fail/stop remain visible.",
  ),
  complianceCase(
    CCTS_CASES.ACTIONS_AGGREGATION,
    "actions-and-control",
    "Aggregation rules stay constrained.",
  ),
  ...
  complianceCase(
    ...
    "lowering-and-ir",
    "Index access and neq semantics are normalized.",
  ),
  complianceCase(
    CCTS_CASES.IR_SYSTEM_LOWERING,
    "lowering-and-ir",
    "Compiler-owned system lowering inserts explicit effects.",
  ),
  complianceCase(
    CCTS_CASES.IR_PLATFORM_NAMESPACE,
    "lowering-and-ir",
    "Platform-owned lowering namespace is tracked.",
  ),
  complianceCase(
    CCTS_CASES.IR_TOTAL_EVALUATION,
    "lowering-and-ir",
    "Expression evaluation stays total.",
  ),
  complianceCase(
    CCTS_CASES.IR_EXPR_CLOSURE,
    "lowering-and-ir",
    "Expression trees remain closed, finite, and flow-free.",
  ),
  complianceCase(
    CCTS_CASES.IR_PURITY_SNAPSHOT_BOUNDARY,
    "lowering-and-ir",
    "Expression roots stay explicit and snapshot-bounded.",
  ),
  complianceCase(
    CCTS_CASES.IR_EVALUATION_ORDER,
    "lowering-and-ir",
    "Expression evaluation order stays left-to-right and key-sorted.",
  ),
  complianceCase(
    CCTS_CASES.IR_PRIMITIVE_EQUALITY,
    "lowering-and-ir",
    "eq/neq stay limited to primitive operands.",
  ),
  complianceCase(
    CCTS_CASES.IR_BOUNDED_SUGAR_ARITH,
    "lowering-and-ir",
    "Bounded arithmetic sugar lowers to existing runtime arithmetic/conditional nodes.",
  ),
  complianceCase(
    CCTS_CASES.IR_MATCH_SUGAR,
    "lowering-and-ir",
    "match() remains finite literal-key branch sugar with source-order lowering.",
  ),
  complianceCase(
    CCTS_CASES.IR_ARG_SELECTION_SUGAR,
    "lowering-and-ir",
    "argmax()/argmin() remain fixed-candidate deterministic selection sugar.",
  ),
  complianceCase(
    CCTS_CASES.IR_SUGAR_DIAGNOSTICS,
    "lowering-and-ir",
    "Bounded sugar shape diagnostics remain visible.",
  ),
  complianceCase(
    CCTS_CASES.IR_OBJECT_SPREAD_LOWERING,
    "lowering-and-ir",
    "Current object-spread lowering and direct merge parity are enforced.",
  ),

  complianceCase(
    CCTS_CASES.FLOW_COMPOSITION,
    "flow-composition",
    "flow/include remains compile-time composition only.",
  ),
  complianceCase(
    CCTS_CASES.FLOW_VALIDATION,
    "flow-composition",
    "Flow declaration and include contracts are tracked.",
  ),

  complianceCase(
    CCTS_CASES.ENTITY_SURFACE,
    "entity-primitives",
    "Entity primitive surface and IR shape are tracked.",
  ),
  complianceCase(
    CCTS_CASES.ENTITY_TRANSFORM_PLACEMENT,
    "entity-primitives",
    "Transform primitive placement remains constrained.",
  ),
  complianceCase(
    CCTS_CASES.ENTITY_TYPING,
    "entity-primitives",
    "Entity typing and uniqueness rules are tracked.",
  ),

  complianceCase(
    CCTS_CASES.INTROSPECTION_GRAPH_SURFACE,
    "introspection",
    "SchemaGraph emits projected nodes with kind-prefixed ids and deterministic ordering.",
  ),
  complianceCase(
    CCTS_CASES.INTROSPECTION_FEEDS_UNLOCKS,
    "introspection",
    "SchemaGraph extracts feeds and unlocks relations from computed deps and availability.",
  ),
  complianceCase(
    CCTS_CASES.INTROSPECTION_MUTATIONS,
    "introspection",
    "SchemaGraph extracts mutates relations from patches and effect into roots.",
  ),
  complianceCase(
    CCTS_CASES.INTROSPECTION_PROJECTION,
    "introspection",
    "SchemaGraph excludes owner namespace substrate and computed nodes that depend on runtime-only roots.",
  ),

  complianceCase(
    CCTS_CASES.SOURCE_EDIT_API_SURFACE,
    "source-editing",
    "compileFragmentInContext API and result semantics are tracked.",
  ),
  complianceCase(
    CCTS_CASES.SOURCE_EDIT_FRAGMENT_GRAMMAR,
    "source-editing",
    "Fragment grammars reject parse failures, scope violations, and raw splice smuggling.",
  ),
  complianceCase(
    CCTS_CASES.SOURCE_EDIT_STALE_MODULE,
    "source-editing",
    "baseModule source-hash validation rejects stale source maps before span reuse.",
  ),
  complianceCase(
    CCTS_CASES.SOURCE_EDIT_RESULT_DETERMINISM,
    "source-editing",
    "Source-edit results keep deterministic text edits, diagnostics, changed targets, and schema diffs.",
  ),
  complianceCase(
    CCTS_CASES.SOURCE_EDIT_TARGET_IMPACT,
    "source-editing",
    "Target validation and impact reporting are compiler-owned and deterministic.",
  ),
  complianceCase(
    CCTS_CASES.SOURCE_EDIT_REMOVE_RENAME_SAFETY,
    "source-editing",
    "Remove and rename safety checks cover complete safe edits and all-or-nothing diagnostic failures.",
  ),

  complianceCase(
    CCTS_CASES.DETERMINISM_COMPILE,
    "determinism",
...
export const COMPILER_RULE_COVERAGE: readonly CompilerComplianceCoverageEntry[] = [
  ...
  ...coverMany(
    ["A20", "A21", "A22", "A23", "A24", "A27", "A34", "AD-COMP-LOW-001"],
    [CCTS_CASES.IR_SYSTEM_LOWERING],
  ),
  ...coverMany(
    ["A25", "E001", "E002"],
    [CCTS_CASES.CONTEXT_COMPUTED_SYSTEM, CCTS_CASES.CONTEXT_STATE_INIT_SYSTEM],
  ),
  ...
  ...coverMany(
    ["STATE-INIT-1", "STATE-INIT-4", "STATE-INIT-5"],
    [CCTS_CASES.STATE_CONSTANT_DEFAULTS],
  ),
  ...coverMany(
    ["STATE-INIT-2", "STATE-INIT-3", "E040", "E041", "E042"],
    [CCTS_CASES.STATE_SCHEMA_REFS],
  ),
  ...coverMany(
    ["COMP-DEP-1", "COMP-DEP-2", "COMP-DEP-3", "COMP-DEP-4", "COMP-DEP-5", "COMP-DEP-6"],
    [CCTS_CASES.STATE_COMPUTED_DEPS, CCTS_CASES.STATE_SCHEMA_REFS],
  ),
  ...coverMany(
    ["TYPE-LOWER-6", "TYPE-LOWER-7", "TYPE-LOWER-8", "TYPE-LOWER-9", "E043", "E044"],
    [CCTS_CASES.STATE_UNSUPPORTED_TYPES],
  ),
  ...
  ...coverMany(
    ["SPREAD-OPERAND-1", "SPREAD-PATCH-1", "SPREAD-PRESENCE-1"],
    [CCTS_CASES.STATE_OBJECT_SPREAD_TYPING],
  ),
  ...coverMany(["SPREAD-CONSUME-1"], [CCTS_CASES.STATE_OBJECT_SPREAD_CONSUME]),
  ...coverMany(
    ["MEL-SUGAR-4"],
    [CCTS_CASES.IR_ARG_SELECTION_SUGAR, CCTS_CASES.STATE_ARG_SELECTION_COVERAGE],
  ),
  ...
  ...coverMany(
    ["MEL-EDIT-1", "MEL-EDIT-2", "MEL-EDIT-3", "MEL-EDIT-4", "MEL-EDIT-13"],
    [CCTS_CASES.SOURCE_EDIT_API_SURFACE],
  ),
  ...coverMany(
    ["MEL-EDIT-5", "MEL-EDIT-6", "E_FRAGMENT_PARSE_FAILED", "E_FRAGMENT_SCOPE_VIOLATION"],
    [CCTS_CASES.SOURCE_EDIT_FRAGMENT_GRAMMAR],
  ),
  ...coverMany(
    ["MEL-EDIT-7", "MEL-EDIT-8", "E_STALE_MODULE"],
    [CCTS_CASES.SOURCE_EDIT_STALE_MODULE],
  ),
  ...coverMany(
    ["MEL-EDIT-9", "MEL-EDIT-14", "MEL-EDIT-15", "MEL-EDIT-16"],
    [CCTS_CASES.SOURCE_EDIT_RESULT_DETERMINISM],
  ),
  ...coverMany(
    ["MEL-EDIT-10", "MEL-EDIT-11", "MEL-EDIT-12", "E_TARGET_NOT_FOUND", "E_TARGET_KIND_MISMATCH"],
    [CCTS_CASES.SOURCE_EDIT_TARGET_IMPACT],
  ),
  ...coverMany(
    ["MEL-EDIT-17", "MEL-EDIT-18", "E_UNSAFE_RENAME_AMBIGUOUS", "E_REMOVE_BLOCKED_BY_REFERENCES"],
    [CCTS_CASES.SOURCE_EDIT_REMOVE_RENAME_SAFETY],
  ),
  ...
  ...coverMany(
    ["ENTITY-2", "ENTITY-2a", "ENTITY-2b", "E030", "E030a", "E030b"],
    [CCTS_CASES.ENTITY_TYPING],
  ),
  ...coverMany(
    [
  ...
  ...coverMany(
    ["SGRAPH-1", "SGRAPH-2", "SGRAPH-3", "SGRAPH-4", "SGRAPH-14", "SGRAPH-15"],
    [CCTS_CASES.INTROSPECTION_GRAPH_SURFACE],
  ),
  ...coverMany(
    ["SGRAPH-5", "SGRAPH-6", "SGRAPH-10", "SGRAPH-11"],
    [CCTS_CASES.INTROSPECTION_FEEDS_UNLOCKS],
  ),
  ...
  ...coverMany(
    ["META-1", "META-2", "META-5", "META-7", "META-8", "META-9", "META-10"],
    [CCTS_CASES.ANNOTATIONS_SURFACE],
  ),
  ...coverMany(["META-6"], [CCTS_CASES.ANNOTATIONS_PAYLOAD]),
  ...coverMany(
    ["META-3", "INV-META-1", "INV-META-2", "INV-META-3", "INV-META-4", "INV-META-5"],
    [CCTS_CASES.ANNOTATIONS_INVARIANTS],
  ),
  ...coverMany(["META-4", "INV-META-6"], [CCTS_CASES.ANNOTATIONS_RUNTIME_BOUNDARY]),
  ...coverMany(
    ["E053", "E054", "E055", "E056", "E057", "E058"],
    [CCTS_CASES.ANNOTATIONS_DIAGNOSTICS],
  ),
