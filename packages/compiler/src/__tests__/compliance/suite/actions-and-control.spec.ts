...
describe("CCTS Actions and Control Suite", () => {
  it(
    caseTitle(CCTS_CASES.ACTIONS_GUARDED_BODY, "(A7) patch/effect statements remain guarded"),
    () => {
    ...
    `);
      ...
      const guardedSatisfied =
        hasDiagnosticCode(patchResult.errors, [
          "E_UNGUARDED_PATCH",
          "E_UNGUARDED_STMT",
          "MEL_PARSER",
        ]) &&
        hasDiagnosticCode(effectResult.errors, [
          "E_UNGUARDED_EFFECT",
          "E_UNGUARDED_STMT",
          "MEL_PARSER",
        ]);
  ...
  it(
    caseTitle(
      ...
      "(A10, COMPILER-MEL-2/2a) once() keeps semantic marker semantics",
    ),
    () => {
    ...
    `);
      ...
      const flow = result.value?.actions["inc"]?.flow as
        ...
        | undefined;
      const thenSeq = flow?.then && flow.then.kind === "seq" ? flow.then : null;
  ...
  it(
    caseTitle(
      ...
      "(A29/A30) fail and stop lower to flow-control nodes",
    ),
    () => {
    ...
    `);
      ...
      expectAllCompliance([
        evaluateRule(
          getRuleOrThrow("A29"),
          failResult.success && failFlow?.kind === "if" && failFlow.then?.kind === "fail",
          {
        ...
        evaluateRule(
          getRuleOrThrow("A30"),
          stopResult.success && stopFlow?.kind === "if" && stopFlow.then?.kind === "halt",
          {
  ...
  it(
    caseTitle(
      ...
      "(E006/E007/E008) fail/stop diagnostics remain visible",
    ),
    () => {
      const failResult = adapter.compile(`
      domain Demo {
  ...
  it(
    caseTitle(CCTS_CASES.ACTIONS_AGGREGATION, "(A32/E009/E010/E011) aggregation stays constrained"),
    () => {
