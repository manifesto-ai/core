...
describe("CCTS Determinism Suite", () => {
  it(
    caseTitle(
      ...
      "(A2/A9) compilation stays deterministic for identical source",
    ),
    () => {
      ...
      const satisfied =
        first.success && second.success && firstHash === secondHash && sameErrors && sameWarnings;
  ...
  it(
    caseTitle(
      CCTS_CASES.DETERMINISM_LOWER,
      "(A9) lowering stays deterministic for identical source",
    ),
    () => {
