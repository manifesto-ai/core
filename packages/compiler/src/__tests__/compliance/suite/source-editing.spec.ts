...
describe("CCTS Source Editing Suite", () => {
  it(
    caseTitle(
      ...
      "(MEL-EDIT-1/2/3/4/13) source-edit API and result semantics are enforced",
    ),
    () => {
      ...
      expectAllCompliance([
        evaluateRule(
          getRuleOrThrow("MEL-EDIT-1"),
          added.ok && added.changedTargets.includes("computed:next"),
          {
            ...
            evidence: [
              noteEvidence("Changed targets", added.changedTargets),
              ...diagnosticEvidence(added.diagnostics),
            ],
          },
        ),
        evaluateRule(
            ...
            hasDiagnosticCode(invalidOp.diagnostics, "E_FRAGMENT_SCOPE_VIOLATION"),
          {
            ...
            evidence: [
              ...diagnosticEvidence(arrayOp.diagnostics),
              ...diagnosticEvidence(invalidOp.diagnostics),
            ],
          },
        ),
        evaluateRule(
          getRuleOrThrow("MEL-EDIT-3"),
          added.ok &&
            added.newSource.includes("domain Demo") &&
            added.newSource.includes("computed next"),
          {
        ...
        evaluateRule(
          ...
          !invalidBase.ok &&
            invalidBase.newSource === "domain Demo { computed nope = " &&
            invalidBase.edits.length === 0,
          {
  ...
  it(
    caseTitle(
      ...
      "(MEL-EDIT-5/6) fragment grammar and raw-splice boundaries are enforced",
    ),
    () => {
      ...
      expectAllCompliance([
        evaluateRule(
          getRuleOrThrow("MEL-EDIT-5"),
          hasDiagnosticCode(invalidExpr.diagnostics, "E_FRAGMENT_PARSE_FAILED"),
          {
        ...
        evaluateRule(
          getRuleOrThrow("E_FRAGMENT_PARSE_FAILED"),
          hasDiagnosticCode(invalidExpr.diagnostics, "E_FRAGMENT_PARSE_FAILED"),
          {
  ...
  it(
    caseTitle(
      ...
      "(MEL-EDIT-7/8) stale baseModule validation is enforced",
    ),
    () => {
      ...
      expectAllCompliance([
        evaluateRule(
          getRuleOrThrow("MEL-EDIT-7"),
          hasDiagnosticCode(result.diagnostics, "E_STALE_MODULE"),
          {
  ...
  it(
    caseTitle(
      ...
      "(MEL-EDIT-9/14/15/16) deterministic edit result semantics are enforced",
    ),
    () => {
      const op = {
        kind: "replaceComputedExpr",
      ...
      expectAllCompliance([
        ...
        evaluateRule(
          ...
          first.edits.length === 1 &&
            first.edits[0]!.range.start.offset !== undefined &&
            first.edits[0]!.range.end.offset !== undefined,
          {
        ...
        evaluateRule(
          getRuleOrThrow("MEL-EDIT-15"),
          applied === first.newSource &&
            first.newSource.startsWith(before) &&
            first.newSource.endsWith(after),
          {
            ...
            evidence: [
              noteEvidence("Applied source", applied),
              noteEvidence("New source", first.newSource),
            ],
          },
        ),
        evaluateRule(
          getRuleOrThrow("MEL-EDIT-16"),
          JSON.stringify(first) === JSON.stringify(second),
          {
            passMessage:
              "Diagnostics, edits, changed targets, and schema diff ordering are stable.",
  ...
  it(
    caseTitle(
      ...
      "(MEL-EDIT-10/11/12) target validation and impact reporting are enforced",
    ),
    () => {
      ...
      expectAllCompliance([
        evaluateRule(
          getRuleOrThrow("MEL-EDIT-10"),
          added.changedTargets.includes("computed:next"),
          {
        ...
        evaluateRule(
          getRuleOrThrow("MEL-EDIT-11"),
          added.schemaDiff?.addedTargets.includes("computed:next") === true,
          {
        ...
        evaluateRule(
          getRuleOrThrow("MEL-EDIT-12"),
          added.schemaDiff !== undefined && added.changedTargets.length > 0,
          {
            passMessage: "Compiler reports impact without deciding author acceptance.",
            failMessage: "Impact reporting was not returned.",
            evidence: [
              noteEvidence("Schema diff", added.schemaDiff),
              noteEvidence("Changed targets", added.changedTargets),
            ],
          },
        ),
        evaluateRule(
          getRuleOrThrow("E_TARGET_NOT_FOUND"),
          hasDiagnosticCode(missing.diagnostics, "E_TARGET_NOT_FOUND"),
          {
        ...
        evaluateRule(
          getRuleOrThrow("E_TARGET_KIND_MISMATCH"),
          hasDiagnosticCode(mismatch.diagnostics, "E_TARGET_KIND_MISMATCH"),
          {
  ...
  it(
    caseTitle(
      ...
      "(MEL-EDIT-17/18) remove/rename all-or-nothing safety is enforced",
    ),
    () => {
    ...
    `;
      const safeRemove = compileFragmentInContext(
        safeRemoveSource,
        {
      ...
      const safeRename = compileFragmentInContext(
        BASE_SOURCE,
        {
      ...
      expectAllCompliance([
        evaluateRule(
            ...
            blockedRename.edits.length === 0,
          {
            ...
            evidence: [
              ...diagnosticEvidence(blockedRemove.diagnostics),
              ...diagnosticEvidence(blockedRename.diagnostics),
            ],
          },
        ),
        evaluateRule(
          ...
          {
            passMessage:
              "Safe remove/rename checks materialize complete source edits and impact reports.",
        ...
        evaluateRule(
          getRuleOrThrow("E_UNSAFE_RENAME_AMBIGUOUS"),
          hasDiagnosticCode(blockedRename.diagnostics, "E_UNSAFE_RENAME_AMBIGUOUS"),
          {
        ...
        evaluateRule(
          getRuleOrThrow("E_REMOVE_BLOCKED_BY_REFERENCES"),
          hasDiagnosticCode(blockedRemove.diagnostics, "E_REMOVE_BLOCKED_BY_REFERENCES"),
          {
