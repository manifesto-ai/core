...
describe("CCTS Annotation Suite", () => {
  it(
    caseTitle(
      ...
      "(META-1/META-2/META-5/META-7/META-8/META-9/META-10) annotation sidecar surface is enforced",
    ),
    () => {
    ...
    `,
      ...
      const tamperedIndex =
        first.module && parsed.program
          ? buildAnnotationIndex(parsed.program, {
              ...first.module.schema,
              actions: {},
            })
          : null;
      const tamperedSourceMap =
        first.module && parsed.program
          ? extractSourceMap(
              parsed.program,
              ANNOTATED_SOURCE,
      ...
      expectAllCompliance([
        evaluateRule(getRuleOrThrow("META-1"), namespaceBlind.errors.length === 0, {
          passMessage:
            "Unknown annotation namespaces compile without compiler-owned semantic interpretation.",
          failMessage: "Compiler no longer treats annotation tags as opaque strings.",
          evidence: diagnosticEvidence(namespaceBlind.errors),
        }),
        evaluateRule(
          getRuleOrThrow("META-2"),
          Boolean(firstModule) &&
            !("annotations" in firstModule.schema) &&
            !("sourceMap" in firstModule.schema) &&
            JSON.stringify(firstModule.graph) ===
              JSON.stringify(extractSchemaGraph(firstModule.schema)),
          {
        ...
        evaluateRule(
          getRuleOrThrow("META-5"),
          Boolean(tamperedIndex && tamperedSourceMap) &&
            hasDiagnosticCode(tamperedIndex!.diagnostics, "E057") &&
            hasDiagnosticCode(tamperedSourceMap!.diagnostics, "E058"),
          {
            passMessage:
              "Tooling sidecar targets are validated against emitted DomainSchema structure.",
            failMessage: "Target validation no longer reports dangling sidecar targets.",
            evidence:
              tamperedIndex && tamperedSourceMap
                ? [
                    ...diagnosticEvidence(tamperedIndex.diagnostics),
                    ...diagnosticEvidence(tamperedSourceMap.diagnostics),
                  ]
                : diagnosticEvidence(first.errors),
          },
        ),
        ...
        evaluateRule(
          getRuleOrThrow("META-8"),
          Boolean(firstModule) &&
            JSON.stringify(firstModule.annotations.entries["computed:canArchive"]) ===
              JSON.stringify([
                { tag: "ui:panel" },
                { tag: "ui:panel" },
            ...
            evidence: firstModule
              ? [
                  noteEvidence(
                    ...
                    firstModule.annotations.entries["computed:canArchive"],
                  ),
                ]
              : diagnosticEvidence(first.errors),
          },
        ...
        evaluateRule(
          getRuleOrThrow("META-9"),
          Boolean(firstModule) && firstModule.annotations.schemaHash === firstModule.schema.hash,
          {
            passMessage: "AnnotationIndex.schemaHash matches DomainSchema.hash.",
            failMessage:
              "AnnotationIndex.schemaHash no longer aligns with the emitted DomainSchema hash.",
            evidence: firstModule
              ? [
        ...
        evaluateRule(
          getRuleOrThrow("META-10"),
          Boolean(firstModule && secondModule) &&
          ...
          {
            passMessage:
              "Tooling sidecar emission is deterministic and omits empty annotation targets.",
  ...
  it(
    caseTitle(
      ...
      "(META-6) annotation payload constraints are enforced",
    ),
    () => {
      ...
      expectAllCompliance([
        evaluateRule(
            ...
            hasDiagnosticCode(invalidDepth.errors, "E056"),
          {
            ...
            failMessage:
              "Annotation payload validation no longer enforces literal-only payloads and depth bounds.",
            evidence: [
  ...
  it(
    caseTitle(
      ...
      "(META-3/INV-META-1..5) annotation erasure invariants are enforced",
    ),
    async () => {
      ...
      const annotated = createManifesto<AnnotationInvariantDomain>(
        annotatedSchema.schema!,
        {},
      ).activate();
      const stripped = createManifesto<AnnotationInvariantDomain>(
        strippedSchema.schema!,
        {},
      ).activate();

      const sameSchema =
        JSON.stringify(annotatedSchema.schema) === JSON.stringify(strippedSchema.schema);
      const sameGraph =
        JSON.stringify(extractSchemaGraph(annotatedSchema.schema!)) ===
        JSON.stringify(extractSchemaGraph(strippedSchema.schema!));
      const annotatedAdmission = annotated.action.archive.check(2);
      const strippedAdmission = stripped.action.archive.check(2);
      const sameDispatchability =
        JSON.stringify(annotatedAdmission) === JSON.stringify(strippedAdmission);
      ...
      const sameAvailability =
        JSON.stringify(annotatedAvailableActions) === JSON.stringify(strippedAvailableActions);
      ...
      expectAllCompliance([
        evaluateRule(
          getRuleOrThrow("META-3"),
          sameSchema && sameGraph && sameDispatchability && sameAvailability && sameSnapshots,
          {
            passMessage: "Structural annotations have zero semantic influence on runtime behavior.",
            failMessage:
              "Structural annotations changed runtime semantics or emitted semantic artifacts.",
            evidence: [
        ...
        evaluateRule(getRuleOrThrow("INV-META-2"), sameGraph, {
          ...
          evidence: [
            noteEvidence(
              "Annotated graph node count",
              extractSchemaGraph(annotatedSchema.schema!).nodes.length,
            ),
            noteEvidence(
              "Stripped graph node count",
              extractSchemaGraph(strippedSchema.schema!).nodes.length,
            ),
          ],
        }),
  ...
  it(
    caseTitle(
      ...
      "(META-4/INV-META-6) DomainModule runtime-boundary guards are enforced",
    ),
    () => {
      ...
      let message = "";
      try {
        createManifesto(result.module as never, {});
      } catch (error) {
        rejected = true;
  ...
  it(
    caseTitle(
      ...
      "(E053-E058) annotation and source-map diagnostic families are enforced",
    ),
    () => {
    ...
    `,
      ...
      const dangling =
        moduleResult.module && parsed.program
          ? buildAnnotationIndex(parsed.program, {
              ...moduleResult.module.schema,
              actions: {},
            })
          : null;
      const danglingSourceMap =
        moduleResult.module && parsed.program
          ? extractSourceMap(
              parsed.program,
              ANNOTATED_SOURCE,
      ...
      expectAllCompliance([
        ...
        evaluateRule(
          getRuleOrThrow("E057"),
          Boolean(dangling) && hasDiagnosticCode(dangling!.diagnostics, "E057"),
          {
            passMessage: "Dangling annotation targets emit E057.",
            failMessage: "Dangling annotation targets no longer emit E057.",
            evidence: dangling
              ? diagnosticEvidence(dangling.diagnostics)
              : diagnosticEvidence(moduleResult.errors),
          },
        ),
        evaluateRule(
          getRuleOrThrow("E058"),
          Boolean(danglingSourceMap) && hasDiagnosticCode(danglingSourceMap!.diagnostics, "E058"),
          {
            passMessage: "Dangling source-map targets emit E058.",
            failMessage: "Dangling source-map targets no longer emit E058.",
            evidence: danglingSourceMap
              ? diagnosticEvidence(danglingSourceMap.diagnostics)
              : diagnosticEvidence(moduleResult.errors),
          },
        ),
      ]);
