...
describe("CCTS Grammar Suite", () => {
  it(
    caseTitle(
      ...
      "(A14/A17/E004) reserved namespaces and identifiers are rejected",
    ),
    () => {
    ...
    `);
      ...
      expectAllCompliance([
        evaluateRule(getRuleOrThrow("A14"), dollarResult.errors.length > 0, {
          ...
          failMessage:
            "The lexer accepted a user-space identifier that crossed the reserved $ namespace boundary.",
  ...
  it(
    caseTitle(
      ...
      "(A5/A6) non-canonical or imperative MEL surface is rejected",
    ),
    () => {
    ...
    `);
      ...
      expectAllCompliance([
        ...
        evaluateRule(getRuleOrThrow("A6"), methodResult.errors.length > 0, {
          passMessage:
            "Method-call sugar remains rejected in favor of canonical MEL function syntax.",
  ...
  it(
    caseTitle(
      ...
      "(E003) invalid runtime references are diagnosed",
    ),
    () => {
  ...
  it(
    caseTitle(
      ...
      "(COMPILER-MEL-3) onceIntent is a contextual keyword",
    ),
    () => {
    ...
    `);
      ...
      const action = statementResult.value?.domain.members.find(
        (member) => member.kind === "action",
      );
      const firstStmt = action && action.kind === "action" ? action.body[0] : null;
      const contextualSatisfied =
        statementResult.success && firstStmt?.kind === "onceIntent" && identifierResult.success;

      expectAllCompliance([
        evaluateRule(getRuleOrThrow("COMPILER-MEL-3"), contextualSatisfied, {
          passMessage:
            "onceIntent is parsed contextually without reserving the identifier globally.",
  ...
  it(
    caseTitle(
      ...
      "(SPREAD-SURFACE-1/SPREAD-DIAG-1) object-spread surface and boundary diagnostics are enforced",
    ),
    () => {
    ...
    `);
      const boundaryDiagnosticsHold = [
        ...
        result.errors.some((diagnostic) =>
          diagnostic.message.includes(
            "Object spread is the only bounded JavaScript-like sugar admitted in current MEL.",
          ),
        ),
      );
      ...
      expectAllCompliance([
        evaluateRule(getRuleOrThrow("SPREAD-SURFACE-1"), surfaceResult.success, {
          passMessage:
            "Object-literal spread is admitted as the bounded v1.2 parser-level shorthand.",
          failMessage:
            "Object-literal spread is not admitted even though SPEC v1.2 declares it current.",
