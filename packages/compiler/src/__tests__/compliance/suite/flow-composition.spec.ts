...
describe("CCTS Flow Composition Suite", () => {
  it(
    caseTitle(
      ...
      "(ADR-013a/A31) flow/include remains compile-time composition only",
    ),
    () => {
      const source = `
      domain Demo {
  ...
  it(
    caseTitle(
      ...
      "(FLOW-PARAM/CALL, E013..E024) flow validation contracts stay visible",
    ),
    () => {
    ...
    `);
      ...
      domain Demo {
        ${Array.from({ length: 17 }, (_, index) => {
          ...
          const target = index === 16 ? `when true { fail "END" }` : `include f${index + 2}()`;
    ...
    `);
      ...
      expectAllCompliance([
        evaluateRule(
          getRuleOrThrow("FLOW-PARAM-1"),
          hasDiagnosticCode(paramConflict.errors, "E021"),
          {
        ...
        evaluateRule(
          getRuleOrThrow("FLOW-PARAM-2"),
          !hasDiagnosticCode(nestedFieldName.errors, "E021"),
          {
        ...
        evaluateRule(
          getRuleOrThrow("FLOW-CALL-2"),
          hasDiagnosticCode(typeMismatch.errors, "E024"),
          {
