...
describe("CCTS Context Suite", () => {
  it(
    caseTitle(
      ...
      "(A25/E001) $system is rejected in computed expressions",
    ),
    () => {
  ...
  it(
    caseTitle(
      ...
      "(A25/E002) $system is rejected in state initializers",
    ),
    () => {
      const result = adapter.compile(`
      domain Demo {
  ...
  it(
    caseTitle(CCTS_CASES.CONTEXT_AVAILABLE_PURITY, "(A28/E005) available when remains schema-pure"),
    () => {
    ...
    `);
      ...
      expectAllCompliance([
        ...
        evaluateRule(getRuleOrThrow("A28"), a28Satisfied, {
          ...
          failMessage:
            "Strengthened v0.7.0 available-context exclusions are not yet fully enforced.",
          evidence: [
