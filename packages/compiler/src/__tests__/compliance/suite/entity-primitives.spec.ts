...
describe("CCTS Entity Primitive Suite", () => {
  it(
    caseTitle(
      ...
      "(ADR-013b, ENTITY-1/3/4/5/7/8/9) entity primitives remain explicit compiler surface",
    ),
    () => {
      const source = `
      domain Demo {
    ...
    `;
      ...
      expectAllCompliance([
        ...
        evaluateRule(getRuleOrThrow("ENTITY-1"), canonicalCallsSatisfied, {
          passMessage:
            "Entity primitives preserve fixed-key entity semantics on the compiler surface.",
        ...
        evaluateRule(getRuleOrThrow("ENTITY-3"), canonicalCallsSatisfied, {
          ...
          failMessage:
            "Entity primitive surface no longer preserves hidden caller-side matching semantics.",
          evidence: [noteEvidence("Compiled schema excerpt", rendered.slice(0, 400))],
        }),
        ...
        evaluateRule(getRuleOrThrow("ENTITY-5"), loweringSatisfied, {
          ...
          failMessage:
            "Entity primitive lowering is not yet expressed in existing Core expression nodes.",
          evidence: [noteEvidence("Lowered schema excerpt", loweredRendered.slice(0, 400))],
        }),
        evaluateRule(getRuleOrThrow("ENTITY-7"), canonicalCallsSatisfied, {
          passMessage:
            "Entity primitives remain a distinct MEL surface instead of generic effect shorthands.",
        ...
        evaluateRule(getRuleOrThrow("ENTITY-9"), canonicalCallsSatisfied, {
          ...
          failMessage:
            "Entity primitives no longer remain as MEL call nodes until the lowering boundary.",
          evidence: [noteEvidence("Compiled schema excerpt", rendered.slice(0, 400))],
        }),
      ]);
    },
  ...
  it(
    caseTitle(
      ...
      "(TRANSFORM-1..5, E031..E035) transform primitives stay placement-constrained",
    ),
    () => {
    ...
    `);
      ...
      expectAllCompliance([
        evaluateRule(
          getRuleOrThrow("TRANSFORM-1"),
          hasDiagnosticCode(computedMisuse.errors, "E031"),
          {
        ...
        evaluateRule(
          getRuleOrThrow("TRANSFORM-2"),
          hasDiagnosticCode(nestedMisuse.errors, "E032"),
          {
        ...
        evaluateRule(
          getRuleOrThrow("TRANSFORM-3"),
          hasDiagnosticCode(computedPathMisuse.errors, "E033"),
          {
            passMessage: "Transform primitive collections must resolve to a state path.",
            failMessage:
              "Transform primitive collections are no longer constrained to state paths.",
        ...
        evaluateRule(
          getRuleOrThrow("TRANSFORM-5"),
          hasDiagnosticCode(availableMisuse.errors, "E035"),
          {
  ...
  it(
    caseTitle(
      ...
      "(ENTITY-2/2a/2b, E030/E030a/E030b) entity typing and uniqueness remain explicit",
    ),
    () => {
    ...
    `);
      ...
      expectAllCompliance([
        ...
        evaluateRule(
          getRuleOrThrow("ENTITY-2a"),
          hasDiagnosticCode(nonPrimitiveId.errors, "E030a"),
          {
        ...
        evaluateRule(
          getRuleOrThrow("ENTITY-2b"),
          hasDiagnosticCode(duplicateInitializer.errors, "E030b"),
          {
        ...
        evaluateRule(
          getRuleOrThrow("E030b"),
          hasDiagnosticCode(duplicateSpreadInitializer.errors, "E030b"),
          {
            passMessage:
              "E030b diagnoses duplicate entity ids introduced through initializer spreads.",
            failMessage:
              "E030b is not emitted for duplicate entity ids introduced through initializer spreads.",
