...
type SystemState = {
  ...
  readonly intentSlots: Record<string, Record<string, unknown>>;
};
```
...
What is **normative** is the behavioral contract each job type must satisfy (§10).
...
| Rule ID | Description |
|---------|-------------|
| REQ-GEN-1 | `requirementId` MUST be computed using §8.2 algorithm |
...
| **FulfillEffect throws** | **CRITICAL: See §13.4** |
...
This appendix provides **example job shapes** for reference implementation. The specific union type is NOT normative; what is normative is the behavioral contract in §10.

### C.1 Reference Job Union

```typescript
// INFORMATIVE: Reference job shapes
// Implementations MAY use different structures as long as §10 rules are satisfied
