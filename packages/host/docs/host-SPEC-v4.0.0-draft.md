...
> **Hard-cut Alignment:** ADR-011, ADR-015, ADR-016
...
|---------|---------|----------|
| v4.0.0 | ADR-015 + ADR-016 epoch boundary draft — remove accumulated `system.errors` from Host-facing Snapshot references, keep Core-owned system transitions on `applySystemDelta()`, and align `$host` boundary semantics to restore normalization | FDR-H025 |
...
10. [Context Determinism](#10-context-determinism)
11. [Invariants](#11-invariants)
12. [Error Handling](#12-error-handling)
13. [Cross-Reference](#13-cross-reference)
- [Appendix A: Quick Reference](#appendix-a-quick-reference)
- [Appendix B: Migration from v1.x](#appendix-b-migration-from-v1x)
- [Appendix C: Job Type Reference (Informative)](#appendix-c-job-type-reference-informative)
- [Appendix D: Deprecated — Compiler Integration](#appendix-d-deprecated--compiler-integration)
...
// ============================================================
// AUTHORITATIVE DEFINITION - Core SPEC v4.0.0 draft
// ============================================================
...
### 3.3.2 Restore Normalization Boundary

When Host receives a snapshot restored through the projected Lineage v2 `restore(worldId)` boundary, it MUST treat that snapshot as already normalized for fresh execution.

Normalization resets transient execution residue before any new Host job begins:

- `data.$host` is treated as reset-at-boundary state
- `system.currentAction` is reset to `null`
- `input` is reset to `null`
- `meta.timestamp` and `meta.randomSeed` are reset and re-provided by Host per job

| Rule ID | Description |
|---------|-------------|
| HOST-RESTORE-1 | Host MUST treat restore-normalized snapshots as boundary-reset for `$host`, `input`, and `system.currentAction` |
| HOST-RESTORE-2 | Host MUST provide fresh per-job `meta.timestamp` / `meta.randomSeed` after restore normalization |
| HOST-RESTORE-3 | Host MUST NOT assume restored snapshots preserve transient Host bookkeeping across resume boundaries |
...
> **Rationale (FDR-H019):** Effect execution MUST be modeled as "job splitting," not "blocking await." `await` inside a job handler creates implicit continuation state — local variables held across await points constitute hidden state outside Snapshot, violating FDR-H003 (No Resume). Job splitting ensures each job reads fresh snapshot with no continuation.
> **Alternatives rejected:** Await inside job handler (continuation state); generators/coroutines (platform-dependent); promise chaining (hidden continuation).
> **Consequences:** Job handlers are synchronous; effect execution splits into request/fulfill cycle; each job reads fresh snapshot.
...
#### 9.2.4 Compute Result and Effect Request Ordering (Critical)
...
### 9.3 Single-Runner Invariant

For each ExecutionKey, only ONE runner processes the mailbox at any time.

> **Rationale (FDR-H020):** For each ExecutionKey, only ONE runner may process the mailbox. Runner MUST re-check mailbox before releasing guard to prevent lost wakeup. Without re-check, effect results arriving during runner shutdown cause the kick to be blocked by the active guard; the runner then exits without seeing the new job, causing permanent stall (LIVE-1 violation). This is not rare — it occurs whenever effect results arrive during runner shutdown.
> **Alternatives rejected:** Lock/mutex (unnecessary complexity); no guard (no enforcement); no re-check (lost wakeup → permanent stall).
> **Consequences:** Single-runner per ExecutionKey; lost wakeup prevented by re-check + `runnerKickRequested` flag.
...
#### 9.3.3 Implementation Pattern (Lost Wakeup Safe)
...
### 9.4 Effect Result Reinjection

Effect results MUST be reinjected to the mailbox as jobs.

> **Rationale (FDR-H021):** Effect results MUST be reinjected to the mailbox as FulfillEffect jobs. Direct application from effect callback bypasses single-writer serialization (FDR-H008), run-to-completion guarantee (FDR-H019), and snapshot version consistency. Reinjection ensures all mutations go through the mailbox.
> **Alternatives rejected:** Direct apply from callback (bypasses single-writer); return value from effect (requires continuation); event emission (non-deterministic ordering).
> **Consequences:** All state mutations serialize through mailbox; effect results traceable as jobs; no hidden mutation paths.
...
### 9.5 Mailbox Liveness Guarantee
...
### 9.6 Effect Result Application Order
...
#### 9.6.2 Effect Execution Policy (MUST choose one)
...
#### 9.6.3 Why Deterministic Order Matters
...
#### 9.6.4 Ordering Buffer Pattern (for ORD-PARALLEL)
...
#### 9.6.5 Timeout/Cancel in Ordering Buffer (ORD-PARALLEL)
...
### 9.7 FulfillEffect Job Contract
...
> **Consequences:** Requirement lifecycle is atomic sequence; FULFILL-0 prevents race conditions; clear happens even on apply failure; skipping any step is SPEC VIOLATION.
...
#### 9.7.3 Implementation (Stale-Safe)
...
#### 9.7.4 Stale Detection Logging (MUST)
...
### 11.2 Intent Processing Invariants
...
### 11.3 Requirement Lifecycle Invariants
...
### 11.4 Execution Model Invariants
...
| INV-SNAP-9 | **Host MUST treat restore-normalized snapshots as fresh execution boundaries for `$host`, `input`, `meta.timestamp`, and `meta.randomSeed`** |
...
### 12.3 Requirement Lifecycle Errors
...
#### 12.4.3 Safe FulfillEffect Pattern (Corrected)
...
| FDR | Topic | Inlined At |
|-----|-------|------------|
| FDR-H001 ~ H011 | Core-Host boundary, Snapshot, Intent, Handlers | §4 ~ §8 |
| FDR-H018 | Execution Mailbox per ExecutionKey | §9.1 |
| FDR-H019 | Run-to-Completion Job Model | §9.2 |
| FDR-H020 | Single-Runner Invariant | §9.3 |
| FDR-H021 | Effect Result Reinjection Protocol | §9.4 |
| FDR-H022 | Requirement Lifecycle Enforcement | §9.7 |
| FDR-H023 | Context Determinism | §10 |
| FDR-H024 | Compiler/Translator Decoupling | Appendix D |
| FDR-H025 | Snapshot Ownership Alignment | §3.3.1 |
| FDR-EL-001 | Event-Loop Execution Model (source) | §9 (motivation) |

### 13.3 App SPEC Alignment
...
| Snapshot | HOST-SNAP-1~4, HOST-NS-1~6, HOST-RESTORE-1~3: Core canonical type, `$host` namespace, restore boundary reset |
...
v2.0.1 **deprecates**:
- Compiler integration (formerly §9, now Appendix D) - Host is decoupled from Compiler/Translator
...
This appendix provides **example job shapes** for reference implementation. The specific union type is NOT normative; what is normative is the behavioral contract in Section 9.

### C.1 Reference Job Union

```typescript
// INFORMATIVE: Reference job shapes
// Implementations MAY use different structures as long as Section 9 rules are satisfied
...
## Appendix D: Deprecated — Compiler Integration
...
*End of Host Contract Specification*
