...
export function createRuntimeKernel<T extends ManifestoDomainShape>({
	...
	if (!initialCanonicalSnapshot) {
		throw new ManifestoError(
			"SCHEMA_ERROR",
			"Host failed to initialize its genesis snapshot",
		);
	}
	...
	function projectSnapshotFromCanonical(
		...
		return cloneAndDeepFreeze(
			projectCanonicalSnapshot<T["state"], T["computed"]>(
				snapshot,
				projectionPlan,
			),
		);
	...
	const actionNames = Object.keys(schema.actions) as Array<
	...
	const actionMetadataByName = Object.freeze(
		Object.fromEntries(
			actionNames.map((name) => {
				...
				const actionRef = refs.actions[name] as unknown as
					| RuntimeActionRef
					| undefined;
				...
				const params = Object.freeze(
					Array.isArray(rawParams)
						? [...rawParams]
						: getActionParamNames(action),
				);
				const publicArity = rawParams === null ? 1 : params.length;
				const requiredArity =
					action.input?.required === false ? 0 : publicArity;
	...
	const getActionMetadata: TypedGetActionMetadata<T> = ((
...
function assertNoReservedPublicActionNames(
	actionNames: readonly string[],
): void {
	const reserved = actionNames.filter((name) =>
