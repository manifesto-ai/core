...
export function createRuntimeStateStore<T extends ManifestoDomainShape>({
	...
	let visibleCanonicalSnapshot: CoreSnapshot = structuredClone(
		initialCanonicalSnapshot,
	);
	...
	function subscribe<R>(
		...
		subscribers.add(
			subscriber as Subscriber<T["state"], T["computed"], unknown>,
		);
		return () => {
			subscribers.delete(
				subscriber as Subscriber<T["state"], T["computed"], unknown>,
	...
	function on<K extends ManifestoEvent>(
		...
		if (!listeners) {
			...
			eventListeners.set(
				event,
				listeners as Set<
					(payload: ManifestoEventPayloadMap[ManifestoEvent]) => void
				>,
			);
		}

		listeners.add(
			handler as (payload: ManifestoEventPayloadMap[ManifestoEvent]) => void,
		);
		return () => {
			listeners?.delete(
				handler as (payload: ManifestoEventPayloadMap[ManifestoEvent]) => void,
	...
	function notifySubscribers(
		snapshot: Snapshot<T["state"], T["computed"]>,
	): void {
