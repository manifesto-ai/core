...
): LineageComposableManifesto<T, L>;
```
...
|---------|-------|-------------|
| LIN-V3-CFG-1 | MUST | `service`, when present, be used as the authoritative backing service |
| LIN-V3-CFG-2 | MUST | when `service` is absent, Lineage MUST create a `DefaultLineageService` from `store` |
| LIN-V3-CFG-3 | MUST | when both `service` and `store` are provided, `service` MUST win |
| LIN-V3-CFG-4 | MUST | when neither `service` nor `store` is provided, Lineage MUST create an in-memory store/service pair |
| LIN-V3-CFG-5 | MUST | `branchId`, when provided, select the runtime continuity branch |
...
| Rule ID | Level | Description |
|---------|-------|-------------|
| LIN-V3-SFC-1 | MUST | `LineageInstance<T>` include the entire base SDK runtime surface except that `dispatchAsync` is promoted to lineage-aware semantics |
...
| LIN-V3-SFC-4 | MUST | `createBranch()` and `switchActiveBranch()` remain lineage-owned runtime verbs, not SDK verbs |
...
| LIN-V3-ACT-3 | MUST | activation construct lineage-aware dispatch and publication from day one |
...
## 4. Lineage-Aware Dispatch

`LineageInstance.dispatchAsync()` means:
...
### 4.3 Failed or Pending Terminal Dispatch
...
| Rule ID | Level | Description |
|---------|-------|-------------|
| LIN-V3-DISPATCH-12 | MUST | if seal prepare or commit fails, `dispatchAsync()` reject |
...
| LIN-V3-DISP-2 | MUST | after dispose, `dispatchAsync`, `restore`, `createBranch`, and `switchActiveBranch` reject with `DisposedError` |
...
- activated lineage runtime promotes `dispatchAsync` instead of duplicating a second execution path.
