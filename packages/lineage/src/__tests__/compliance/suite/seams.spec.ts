  ...
  it(
    ...
    () => {
      ...
      const hasLineageSurface =
        typeof exported.withLineage === "function" &&
      ...
      const omitsGovernanceSurface =
        !("createProposal" in exported) &&
        !("createDecisionRecord" in exported) &&
        !("createNoopWorldEventSink" in exported) &&
        !("ProposalQueue" in exported);
      const governanceImports = importSpecifiers.filter((specifier) =>
        specifier.startsWith("@manifesto-ai/governance"),
      );
      const eventImports = importSpecifiers.filter(
        (specifier) =>
          specifier === "events" || specifier === "node:events" || specifier.includes("/events"),
      );
      ...
      expectAllCompliance([
        evaluateRule(getRuleOrThrow("LIN-BOUNDARY-1"), governanceImports.length === 0, {
          ...
          evidence: [
            noteEvidence(
              "Verified source imports directly instead of inferring from export shape.",
            ),
          ],
        }),
        evaluateRule(
          getRuleOrThrow("LIN-BOUNDARY-4"),
          eventImports.length === 0 && eventEmissionSites === 0,
          {
            ...
            evidence: [
              noteEvidence(
                "Scanned source imports and common event-emission primitives to verify event-free lineage ownership.",
              ),
            ],
          },
        ),
        evaluateRule(
          ...
          {
            passMessage:
              "Lineage package provides an in-memory store with branch and attempt query seams.",
            failMessage:
              "In-memory LineageStore implementation is missing branch/tip/attempt capabilities.",
            evidence: [
              noteEvidence("Verified core store methods on createInMemoryLineageStore()."),
            ],
          },
        ),
      ]);
