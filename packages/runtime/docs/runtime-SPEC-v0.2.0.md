...
This document defines the **Manifesto Runtime Specification v0.2.0**.
...
| `createRuntime()` factory | SDK SPEC responsibility |
...
### 5.6.1 Persisted Patch Format Validation (ADR-009)

Runtime persistence adapters MUST validate serialized patch payload format at store/restore boundaries.
...
| Rule ID | Level | Description |
|---------|-------|-------------|
| RT-PATCHFMT-1 | MUST | Serialized patch payload MUST include `_patchFormat` |
| RT-PATCHFMT-2 | MUST | Runtime restore boundary MUST accept only `_patchFormat: 2` |
| RT-PATCHFMT-3 | MUST | `_patchFormat: 1` or missing tag MUST be hard-rejected |
| RT-PATCHFMT-4 | MUST | On legacy payload rejection, runtime MUST fall back to fresh genesis initialization (epoch reset) |
...
2. If failed, reflect error state through Core-owned system transition channel (`SystemDelta.lastError`)
...
| SDK SPEC | v0.2.0 | Public API layer (companion spec) |
...
*End of Manifesto Runtime Specification v0.2.0*
