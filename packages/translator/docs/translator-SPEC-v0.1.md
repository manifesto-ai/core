...
// Conceptual edge: n2 → n1 (n2 depends on n1)
// Execution order: n1 must complete before n2
```

**Convention:** `dependsOn[i]` means "this node depends on node with id `dependsOn[i]`".
...
| **I2. Referential Identity** | Entity refs within graph maintain identity | `translate()` MUST verify |
| **I3. Conceptual Completeness** | Missing args are explicitly recorded | `translate()` MUST verify |
...
INVARIANT: For all nodes n in graph G:
  - If required θ-role (from Lexicon's theta frame) is unbound:
    - n.resolution.missing MUST contain the role name (e.g., "TARGET", "THEME")
...
|-------|------|------------------|--------|
| **Structural** | `translate()` return | No | I1, I2 (structural), I3 (structural), I4 |
| **Lexicon-Verified** | `validate()` call | Yes | I2 (type), I3 (type), Feature Checking |

**Scope Clarification:**
- **I2 (Referential Identity):**
    - *Structural:* Same symbolic ref (e.g., `{ kind: "that" }`) used consistently within graph
    - *Lexicon-Verified:* `entityType` exists in Lexicon's entity registry
- **I3 (Conceptual Completeness):**
    - *Structural:* If `missing` is non-empty, `status` is not "Resolved"
    - *Lexicon-Verified:* `missing` matches Lexicon's `thetaFrame.required`, selectional restrictions satisfied
...
function validateStructural(graph: IntentGraph): ValidationResult {
  ...
  for (const node of graph.nodes) {
    ...
    if (typeof node.resolution.ambiguityScore !== "number" ||
      ...
      return { valid: false, error: "INVALID_SCORE" };
    }
  }
  ...
  for (const node of graph.nodes) {
    for (const dep of node.dependsOn) {
      if (!nodeIds.has(dep)) {
        return { valid: false, error: "BROKEN_EDGE" };
      }
    }
  }
...
function validate(
  ...
  for (const node of graph.nodes) {
    ...
    if (missingRoles.length > 0) {
      ...
      if (!arraysEqual(node.resolution.missing ?? [], missingRoles)) {
        return { valid: false, error: "MISSING_MISMATCH", nodeId: node.id };
      }
    }
...
> **Note:** `translate()` is asynchronous because it may invoke an LLM provider for natural language processing. If no LLM provider is configured, a stub implementation returns an empty graph with a warning.
...
Resolver MAY maintain internal state (e.g., execution results) to resolve cross-step references:
...
class StatefulResolver implements Resolver {
  ...
  resolveReferences(ir: IntentIR): ResolvedIntentIR | { deferred: true; reason: string } {
    // Use executionResults to resolve "that project" → concrete ID
  }
}
```

### 10.5 LLM Provider Interface

```typescript
/**
 * LLM Provider for natural language translation.
 * Implementations connect to different LLM backends (OpenAI, Anthropic, etc.)
 */
interface LLMProvider {
  /** Provider name (e.g., "openai", "anthropic") */
  readonly name: string;

  /** Translate natural language to Intent Graph nodes */
...
type LLMIntentNode = {
  /** Temporary ID for dependency tracking */
  readonly tempId: string;

  /** Generated IntentIR */
  readonly ir: IntentIR;

  /** Dependencies (tempIds of other nodes) */
  readonly dependsOnTempIds: string[];
...
type AmbiguityIndicators = {
  /** Whether the node has unresolved discourse references */
  readonly hasUnresolvedRef: boolean;

  /** Required θ-roles that are missing */
  readonly missingRequiredRoles: Role[];

  /** Whether multiple interpretations are possible */
  readonly multipleInterpretations: boolean;

  /** LLM's confidence score (0-1) */
...
import { createOpenAIProvider } from "@manifesto-ai/translator";
...
**Default Behavior:**
- If `options.llm.provider` is not provided, uses environment-configured provider
- Falls back to stub (empty graph with warning) if no LLM is configured
...
INVARIANT: For all i, j where steps[i].nodeId depends on steps[j].nodeId:
  i > j (dependency comes first in the array)
```
...
| Level | Requirements |
|-------|--------------|
| **Minimal** | `translate()` returns valid IntentGraph |
...
1. `translate(text)` returns `TranslateResult` with valid `IntentGraph`
...
|---------|------|---------|
| 0.1.1 | 2026-01-26 | Breaking: `translate()` is now async (returns `Promise<TranslateResult>`), LLM provider integration, added §10.5 LLM Provider Interface |
