...
const ProductsListWrapper = defineComponent({
  ...
  setup(props) {
    const userRole = ref(props.initialRole)
    const actionLog = ref<string[]>([])

    // Computed context for proper change detection
    const context = computed<Partial<EvaluationContext>>(() => ({
      user: { role: userRole.value },
    }))

    const handleRowClick = (rowId: string, row: Record<string, unknown>) => {
      actionLog.value = [...actionLog.value, `Row clicked: ${row.name} (${rowId})`]
    }
    ...
    const handleRowAction = (
      ...
      actionLog.value = [...actionLog.value, `Action ${actionId}: ${row.name}`]
    }
    ...
    return () =>
      h('div', { style: { padding: '2rem', maxWidth: '1200px', margin: '0 auto' } }, [
        ...
        h(
          ...
          [
            h('h1', '상품 목록'),
            h('p', { style: { color: '#666' } }, 'ListRenderer E2E 테스트'),
              ...
              [
                ...
                h(
                  'select',
                  {
                    ...
                    onChange: (e: Event) => {
                      userRole.value = (e.target as HTMLSelectElement).value
                    },
                    style: { padding: '0.5rem', borderRadius: '4px' },
                  },
                  [
                    h('option', { value: 'user' }, '일반 사용자'),
        ...
        actionLog.value.length > 0
          ? h(
              ...
              [
                h('h3', { style: { margin: '0 0 0.5rem 0' } }, 'Action Log'),
                h(
                  'ul',
                  { style: { margin: 0, paddingLeft: '1.5rem' } },
                  actionLog.value.slice(-5).map((log, i) => h('li', { key: i }, log))
                ),
              ]
            )
          : null,
