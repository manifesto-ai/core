/**
 * Logistics Services - Mock handlers for shipping domain
 */
...
const recordFilter = async (params: Record<string, unknown>): Promise<Patch[]> => {
  const source = params.source as Record<string, unknown> | null;
  const into = params.into as string;
  return [{ op: "set", path: into, value: source ?? {} }];
};

const recordMapValues = async (params: Record<string, unknown>): Promise<Patch[]> => {
  const into = params.into as string;
  return [{ op: "set", path: into, value: {} }];
};

const arrayFilter = async (params: Record<string, unknown>): Promise<Patch[]> => {
